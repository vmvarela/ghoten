...
test-pg: ## Runs tests with local Postgres instance as the backend.
	...
	@ make test-pg-clean
	@ docker run --rm -d --name ghoten-pg \
        -p $(PG_PORT):5432 \
        -e POSTGRES_PASSWORD=ghoten \
        -e POSTGRES_USER=ghoten \
        postgres:16-alpine3.17 1> /dev/null
	@ docker exec ghoten-pg /bin/sh -c 'until psql -U ghoten -c "\q" 2> /dev/null; do echo "Database is getting ready, waiting"; sleep 1; done'
	@ DATABASE_URL="postgres://ghoten:ghoten@localhost:$(PG_PORT)/ghoten?sslmode=disable" \
...
test-consul: ## Runs tests using in docker container using Consul as the backend.
	...
	@ cd ./internal/backend/remote-state/consul &&\
  		docker build --build-arg="GO_VERSION=${GO_VER}" -t ghoten-consul --progress=plain . &> /dev/null
	@ echo "Run tests"
	@ docker run --rm --name ghoten-consul -v $(PWD):/app -e TF_CONSUL_TEST=1 -t ghoten-consul \
 		test ./internal/backend/remote-state/consul/...

test-consul-clean: ## Cleans environment after `test-consul`.
	@ docker rmi -f ghoten-consul:latest
...
test-kubernetes: test-kubernetes-clean ## Runs tests with a local kubernetes cluster as the backend.
	...
	@ /tmp/tofuk8s -q create cluster --name ghoten-kubernetes
	@ /tmp/tofuk8s -q export kubeconfig --name ghoten-kubernetes --kubeconfig /tmp/ghoten-k8s-config
	@ echo "Running tests"
	@ KUBE_CONFIG_PATHS=/tmp/ghoten-k8s-config TF_K8S_TEST=1 go test ./internal/backend/remote-state/kubernetes/...
...
test-kubernetes-clean: ## Cleans environment after `test-kubernetes`.
	@ test -s /tmp/ghoten-k8s-config && rm /tmp/ghoten-k8s-config || echo "" > /dev/null
	@ test -s /tmp/tofuk8s && (/tmp/tofuk8s -q delete cluster --name ghoten-kubernetes && rm /tmp/tofuk8s) || echo "" > /dev/null
