...
// index manifest for an Ghoten provider version.
//
// If a selected tag refers to a manifest that either lacks an artifact type
// or has a different artifact type then Ghoten will reject it with an
// error indicating that it seems to be something other than an Ghoten provider.
...
//
// Ghoten will silently ignore any listed descriptors that either lack an artifact
// type or use a different one, both so that future versions of Ghoten can
// be sensitive to additional artifact types if needed and so that those creating
// an artifact can choose to blend in other non-Ghoten-related artifacts if
...
// package is built for. Ghoten and OCI both use Go's codenames for operating
...
//
// Ghoten does not currently make any use of the other properties defined for
// a "platform" object, and so will silently ignore any descriptors that set
// those properties. Future versions of Ghoten might be able to use finer-grain
...
// acts as a client for the Ghoten-specific "provider mirror protocol"
...
type OCIRegistryMirrorSource struct {
	...
	// Ghoten-style provider source addresses into OCI Distribution
	...
	// that Ghoten-style provider source addresses support a
...
func (o *OCIRegistryMirrorSource) AvailableVersions(ctx context.Context, provider addrs.Provider) (VersionList, Warnings, error) {
	ctx, span := tracing.Tracer().Start(
		...
		tracing.SpanAttributes(
			traceattrs.GhotenProviderAddress(provider.String()),
		),
	)
	defer span.End()
...
func (o *OCIRegistryMirrorSource) PackageMeta(ctx context.Context, provider addrs.Provider, version Version, target Platform) (PackageMeta, error) {
	ctx, span := tracing.Tracer().Start(
		...
		tracing.SpanAttributes(
			traceattrs.GhotenProviderAddress(provider.String()),
			traceattrs.GhotenProviderVersion(version.String()),
			traceattrs.GhotenTargetPlatform(target.String()),
		),
	)
	defer span.End()
	...
	// we fetched to suit Ghoten's provider installer API.

	// We'll announce the Ghoten-style package hash that we're expecting as part of
	...
	return PackageMeta{
		...
		// that's okay because we don't care very much about it in Ghoten
		...
		// Ghoten provider network mirror.
		...
		// any of the protocol versions that this Ghoten version supports.
		// Omitting this field is okay though, since some other mirror sources
		// can't support it either: that just means that Ghoten will discover
...
func fetchOCIDescriptorForVersion(ctx context.Context, version versions.Version, store OCIRepositoryStore) (ociv1.Descriptor, error) {
	...
	span.SetAttributes(
		...
		traceattrs.GhotenOCIReferenceTag(tagName),
		traceattrs.GhotenOCIManifestMediaType(desc.MediaType),
	)
	...
	if desc.ArtifactType != "" && desc.ArtifactType != ociIndexManifestArtifactType {
		span.SetAttributes(
			traceattrs.GhotenOCIManifestArtifactType(desc.ArtifactType),
		)
		...
		case "application/vnd.opentofu.provider-target":
			...
			return desc, prepErr(fmt.Errorf("tag refers directly to image manifest, but Ghoten providers require an index manifest for multi-platform support"))
		case "application/vnd.opentofu.modulepkg":
			...
			// and modules is common for those new to Ghoten terminology.
			return desc, prepErr(fmt.Errorf("selected OCI artifact is an Ghoten module package, not a provider package"))
		default:
...
func fetchOCIIndexManifest(ctx context.Context, desc ociv1.Descriptor, store OCIRepositoryStore) (*ociv1.Index, error) {
	ctx, span := tracing.Tracer().Start(
		...
		tracing.SpanAttributes(
			traceattrs.OCIManifestDigest(desc.Digest.String()),
			traceattrs.GhotenOCIManifestSize(desc.Size),
		),
	)
	defer span.End()
	prepErr := func(err error) error {
	...
	span.SetAttributes(
		traceattrs.GhotenOCIManifestMediaType(manifest.MediaType),
		traceattrs.GhotenOCIManifestArtifactType(manifest.ArtifactType),
	)
	...
	// to extend this format in backward-compatible ways in future Ghoten versions.
	return &manifest, nil
}
...
func selectOCIImageManifest(descs []ociv1.Descriptor, provider addrs.Provider, version versions.Version, target Platform) (ociv1.Descriptor, error) {
	...
	for _, desc := range descs {
		...
		if desc.Platform.OSVersion != "" || desc.Platform.OS != target.OS || desc.Platform.Architecture != target.Arch {
			...
			// future Ghoten versions the option of treating that as a more specific
	...
	if foundManifests == 0 {
		...
		default:
			...
			return selected, fmt.Errorf("provider image manifest has unsupported artifact type; is this a container image, rather than an Ghoten provider?")
		}
	}
	if foundManifests > 1 {
...
func selectOCILayerBlob(descs []ociv1.Descriptor) (ociv1.Descriptor, error) {
	...
	for _, desc := range descs {
		if desc.MediaType != ociPackageMediaType {
			...
			// artifact type and media type so that future versions of Ghoten
			// can potentially support additional archive formats, and so that
			// artifact authors can include other non-Ghoten-related layers
	...
	if foundBlobs == 0 {
		if foundWrongMediaTypeBlobs > 0 {
			return selected, fmt.Errorf("image manifest contains no layers of type %q, but has other unsupported formats; this OCI artifact might be intended for a different version of Ghoten", ociPackageMediaType)
		}
...
func fetchOCIManifestBlob(ctx context.Context, desc ociv1.Descriptor, store OCIRepositoryStore) ([]byte, error) {
	...
	if (desc.Size / 1024 / 1024) > ociImageManifestSizeLimitMiB {
		return nil, fmt.Errorf("manifest size exceeds Ghoten's size limit of %d MiB", ociImageManifestSizeLimitMiB)
	}
