...
import (
	...
	"github.com/vmvarela/ghoten/internal/collections"
)
...
const (
	enforceGPGValidationEnvName = "GHOTEN_ENFORCE_GPG_VALIDATION"
	enforceGPGExpirationEnvName = "GHOTEN_ENFORCE_GPG_EXPIRATION"
)
...
// given hashes are considered acceptable by the current version of Ghoten.
//
...
// hashes are considered by Ghoten to be the strongest verification, and
...
func (a packageHashAuthentication) AuthenticatePackage(localLocation PackageLocation) (*PackageAuthenticationResult, error) {
	if len(a.RequiredHashes) == 0 {
		...
		// version of Ghoten.
		return nil, fmt.Errorf("this version of Ghoten does not support any of the checksum formats given for this provider")
	}
...
//
// Ghoten requires a valid GPG signature for any provider for which this
...
// compromise to allow the main Ghoten registry to serve providers for
// which it does not currently know a signing key. For more information,
// refer to:
//
//	https://github.com/vmvarela/ghoten/issues/266
//
...
func ShouldEnforceGPGValidationForProvider(addr addrs.Provider) bool {
	...
	// Ghoten registry, since our possibility of skipping verification
...
func (s signatureAuthentication) AuthenticatePackage(location PackageLocation) (*PackageAuthenticationResult, error) {
	...
	if shouldValidate {
		...
		log.Printf("[WARN] Skipping GPG validation of provider package %s as no keys were provided by the registry. See https://github.com/vmvarela/ghoten/pull/309 for more information.", location)
	}
...
func (s signatureAuthentication) findSigningKey() (*SigningKey, string, error) {
	...
	if expiredKey != nil && !s.shouldEnforceGPGExpiration() {
		fmt.Printf("[WARN] Provider %s/%s (%v) gpg key expired, this will fail in future versions of Ghoten\n",
