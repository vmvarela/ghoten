...
// so that future versions of Ghoten can potentially support provider packages
...
type PackageOCIBlobArchive struct {
	...
	// "application/vnd.opentofu.providerpkg"; future Ghoten formats might support
...
func (p PackageOCIBlobArchive) InstallProviderPackage(ctx context.Context, meta PackageMeta, targetDir string, allowedHashes []Hash) (*PackageAuthenticationResult, error) {
	...
	ctx, span := tracing.Tracer().Start(
		...
		tracing.SpanAttributes(
			traceattrs.GhotenOCIRegistryDomain(p.registryDomain),
			traceattrs.GhotenOCIRepositoryName(p.repositoryName),
			traceattrs.GhotenOCIBlobDigest(pkgDesc.Digest.String()),
			traceattrs.GhotenOCIBlobMediaType(pkgDesc.MediaType),
			traceattrs.GhotenOCIBlobSize(pkgDesc.Size),
			traceattrs.String("opentofu.provider.local_dir", targetDir),
			traceattrs.GhotenProviderAddress(meta.Provider.String()),
			traceattrs.GhotenProviderVersion(meta.Version.String()),
			traceattrs.GhotenTargetPlatform(meta.TargetPlatform.String()),
		),
	)
	defer span.End()
...
func fetchOCIBlobToTemporaryFile(ctx context.Context, desc ociv1.Descriptor, store orasContent.Fetcher) (loc PackageLocalArchive, err error) {
	...
	// We'll eventually need to generate an Ghoten-style hash for this package anyway,
...
func hashFromOCIDigest(digest ociDigest.Digest) (Hash, error) {
	...
	if algo := digest.Algorithm(); algo != ociDigest.SHA256 {
		// Ghoten's "ziphash" format requires a SHA256 checksum in particular.
...
func checkOCIBlobDescriptor(desc ociv1.Descriptor, meta PackageMeta) error {
	...
	if desc.Platform != nil {
		...
		if desc.Platform.OS != meta.TargetPlatform.OS || desc.Platform.Architecture != meta.TargetPlatform.Arch {
			// We'll use our own type here to produce an Ghoten-conventional string representation
