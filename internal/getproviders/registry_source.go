...
func (s *RegistrySource) AvailableVersions(ctx context.Context, provider addrs.Provider) (VersionList, Warnings, error) {
	...
	// Ghoten becomes relevant only once we've selected one, at which point
	...
	// action such as upgrading Ghoten, using a different OS to run
	// Ghoten, etc. Changes that affect compatibility are considered breaking
