...
func TestOCIRegistryMirrorSource(t *testing.T) {
	...
	// this one is irrelevant because Ghoten can't interact with it.
	...
	source := &OCIRegistryMirrorSource{
		...
		getOCIRepositoryStore: func(ctx context.Context, registryDomain, repositoryName string) (OCIRepositoryStore, error) {
			...
			case "not_provider":
				...
				// of not-actually-Ghoten-provider artifacts.
	...
	t.Run("happy path", func(t *testing.T) {
		...
		if got, want := meta.Filename, "terraform-provider-bar_1.0.1_amigaos_m86k.zip"; got != want {
			// NOTE: This field doesn't actually really matter; Ghoten doesn't
	...
	t.Run("module package instead of provider", func(t *testing.T) {
		...
		if got, want := err.Error(), `selected OCI artifact is an Ghoten module package, not a provider package`; got != want {
	...
	t.Run("tag directly refers to image manifest, not index manifest", func(t *testing.T) {
		...
		if got, want := err.Error(), `tag refers directly to image manifest, but Ghoten providers require an index manifest for multi-platform support`; got != want {
			t.Errorf("wrong error\ngot:  %s\nwant: %s", got, want)
		}
	})
	t.Run("valid manifests but unsupported archive format", func(t *testing.T) {
		...
		if got, want := err.Error(), `image manifest contains no layers of type "archive/zip", but has other unsupported formats; this OCI artifact might be intended for a different version of Ghoten`; got != want {
			t.Errorf("wrong error\ngot:  %s\nwant: %s", got, want)
		}
	})
}
...
func makeFakeOCIRepositoryWithNonProviderContent(t *testing.T) (OCIRepositoryStore, *fakeOCIRepositoryContent) {
	...
	// that don't match Ghoten's provider package manifest layout, so that
	...
	// formats used by other Ghoten features, some on other software that
	...
	{
		// a manifest that follows our conventions for Ghoten module packages, rather
