...
func TestCloud_planWithRequiredVariables(t *testing.T) {
	...
	op.Variables = testVariables(ghoten.ValueFromCLIArg, "foo") // "bar" variable defined in config is  missing
	op.Workspace = testBackendSingleWorkspaceName
...
func TestCloud_planWithWorkingDirectory(t *testing.T) {
	...
	op, done := testOperationPlan(t, "./testdata/plan-with-working-directory/ghoten")
	defer done(t)
...
func TestCloud_planWithWorkingDirectoryFromCurrentPath(t *testing.T) {
	...
	t.Chdir("./testdata/plan-with-working-directory/ghoten")
