...
import (
	...
	"github.com/vmvarela/ghoten/internal/command/jsonformat"
...
// integration for Ghoten CLI. This backend is not intended to be surfaced at the user level and
...
func (b *Cloud) Configure(ctx context.Context, obj cty.Value) tfdiags.Diagnostics {
	...
	if parseErr != nil || currentAPIVersion.LessThan(desiredAPIVersion) {
		...
		if b.runningInAutomation {
			// It should never be possible for this Ghoten process to be mistakenly
...
func (b *Cloud) setConfigurationFields(obj cty.Value) tfdiags.Diagnostics {
	...
	if val := obj.GetAttr("hostname"); !val.IsNull() && val.AsString() != "" {
		...
		diags = diags.Append(tfdiags.Sourceless(
			...
			`Ghoten does not provide a default "hostname" attribute, so it must be set to the hostname of the cloud backend.`,
		))
...
func (b *Cloud) StateMgr(ctx context.Context, name string) (statemgr.Full, error) {
	...
	if err == tfe.ErrResourceNotFound {
		...
		// Attempt to set the new workspace to use this version of Ghoten. This
	...
	if !b.ignoreVersionConflict {
		...
		if remoteTFVersion != tfversion.String() && remoteTFVersion != "latest" {
			return nil, fmt.Errorf("Remote workspace TF version %q does not match local Ghoten version %q", remoteTFVersion, tfversion.String())
		}
	}
...
// the workspace's configured Ghoten version. If they are compatible, this
...
func (b *Cloud) VerifyWorkspaceTerraformVersion(workspaceName string) tfdiags.Diagnostics {
	...
	// If the workspace has execution-mode set to local, the remote Ghoten
	...
	if err != nil {
		message := fmt.Sprintf(
			"The remote workspace specified an invalid TF version or constraint (%s), "+
				"and it isn't possible to determine whether the local Ghoten version (%s) is compatible.",
	...
	message := fmt.Sprintf(
		"The local Ghoten version (%s) does not meet the version requirements for remote workspace %s/%s (%s).",
		tfversion.String(),
		b.organization,
		workspace.Name,
		remoteConstraint,
...
func (b *Cloud) validWorkspaceEnvVar(ctx context.Context, organization, workspace string) tfdiags.Diagnostic {
	...
	if err == tfe.ErrResourceNotFound {
		return tfdiags.Sourceless(
			...
			fmt.Sprintf(`Ghoten failed to find workspace %q in organization %s.`, workspace, organization),
		)
	}
	...
	if b.WorkspaceMapping.Strategy() == WorkspaceTagsStrategy {
		...
		return tfdiags.Sourceless(
			...
			"Invalid workspace selection",
			fmt.Sprintf(
				"Ghoten failed to find workspace %q with the tags specified in your configuration:\n[%s]",
				workspace,
...
[reset][yellow]The local Ghoten version (%s) is not available in the cloud backend, or your
...
Please reach out to Ghoten Support to resolve this issue.`

var (
	workspaceConfigurationHelp = fmt.Sprintf(
		`The 'workspaces' block configures how Ghoten CLI maps its workspaces for this single
