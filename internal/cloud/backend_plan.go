...
import (
	...
	"github.com/vmvarela/ghoten/internal/cloud/cloudplan"
	...
	if !op.HasConfig() && op.PlanMode != plans.DestroyMode {
		diags = diags.Append(tfdiags.Sourceless(
			...
			`Plan requires configuration to be present. Planning without a configuration `+
				...
				`a Ghoten configuration file in the path being executed and try again.`,
		))
	}
...
Ghoten will upload the contents of the following directory,
...
func (b *Cloud) AssertImportCompatible(config *configs.Config) error {
	...
	if len(config.Module.Import) > 0 && os.Getenv("TFC_RUN_ID") != "" {
		...
		if err != nil {
			return fmt.Errorf("Error parsing remote API version. To proceed, please remove any import blocks from your config. Please report the following error to the Ghoten team: %w", err)
		}
		...
		if !isSet {
			return fmt.Errorf("Error reading TFC agent version. To proceed, please remove any import blocks from your config. Please report the following error to the Ghoten team: TFC_AGENT_VERSION not present.")
		}
		currentAgentVersion, err := version.NewVersion(agentEnv)
		if err != nil {
			return fmt.Errorf("Error parsing TFC agent version. To proceed, please remove any import blocks from your config. Please report the following error to the Ghoten team: %w", err)
		}
