...
func testRunner(t *testing.T, cases testCases, orgCount int, tfEnvFlags ...string) {
	for name, tc := range cases {
		...
		t.Run(name, func(subtest *testing.T) {
			...
			tf := e2e.NewBinary(t, ghotenBin, tmpDir)
...
func setupBinary() func() {
	...
	cmd := exec.Command(
		...
		"-ldflags", fmt.Sprintf("-X \"github.com/vmvarela/ghoten/version.Prerelease=%s\"", tfversion.Prerelease),
		"./cmd/ghoten",
	)
	err = cmd.Run()
	if err != nil {
	...
	ghotenBin = fmt.Sprintf("%s/terraform", tmpTerraformBinaryDir)
