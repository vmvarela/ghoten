...
import (
	...
	copydir "github.com/vmvarela/ghoten/internal/copy"
...
type Installer struct {
	...
	// namespace, which we use for providers that are built in to Ghoten
	...
	// considered implemented, but that Ghoten does not manage the
...
// to Ghoten running on another operating system or CPU architecture.
...
// built in to Ghoten and thus do not need a separate installation step.
//
...
// indicated by the passed addrs.Providers as unmanaged. Ghoten does not
// need to control the lifecycle of these providers, and they are assumed to be
// running already when Ghoten is started. Because these are essentially
// processes, not binaries, Ghoten will not do any work to ensure presence
...
func (i *Installer) EnsureProviderVersions(ctx context.Context, locks *depsfile.Locks, reqs getproviders.Requirements, mode InstallMode) (*depsfile.Locks, error) {
	...
	span.SetAttributes(traceattrs.GhotenTargetPlatform(targetPlatform.String()))
		...
		if provider.IsBuiltIn() {
			...
			if valid {
				if len(versionConstraints) == 0 {
					...
					// that is built in to the current Ghoten release.
					err = fmt.Errorf("built-in providers do not support explicit version constraints")
				}
			} else {
				err = fmt.Errorf("this Ghoten release has no built-in provider named %q", provider.Type)
			}
			if err != nil {
				errs[provider] = err
		...
		wg.Go(func() {
			traceCtx, span := tracing.Tracer().Start(ctx,
				...
				tracing.SpanAttributes(
					...
					traceattrs.GhotenTargetPlatform(targetPlatform.String()),
				),
			)
			defer span.End()
...
func (i *Installer) ensureProviderVersionInstalled(
	...
	if linkTo != nil {
		...
		if new == nil {
			err := fmt.Errorf("after installing %s it is still not detected in %s; this is a bug in Ghoten", provider, linkTo.BasePath())
			if cb := evts.LinkFromCacheFailure; cb != nil {
				cb(provider, version, err)
			}
			return nil, nil, err
...
func (i *Installer) ensureProviderVersionInDirectory(
	...
	new := installTo.ProviderVersion(provider, version)
	if new == nil {
		err := fmt.Errorf("after installing %s it is still not detected in %s; this is a bug in Ghoten", provider, installTo.BasePath())
		if cb := evts.FetchPackageFailure; cb != nil {
			cb(provider, version, err)
		}
		return nil, nil, err
