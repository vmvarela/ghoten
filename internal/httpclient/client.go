...
// package that will also send a Ghoten User-Agent string.
//
...
func New(ctx context.Context) *http.Client {
	...
	cli.Transport = &userAgentRoundTripper{
		userAgent: GhotenUserAgent(version.Version),
		inner:     cli.Transport,
	}
	...
	if span := tracing.SpanFromContext(ctx); span != nil && span.IsRecording() {
		...
		// the Ghoten process -- as sufficient signal that generating
