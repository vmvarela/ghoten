...
func testSession(t *testing.T, test testSessionTest) {
	...
	ctx, diags := ghoten.NewContext(&ghoten.ContextOpts{
	...
	scope, diags := ctx.Eval(context.Background(), config, state, addrs.RootModuleInstance, &ghoten.EvalOpts{})
	if diags.HasErrors() {
