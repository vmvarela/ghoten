...
// TestCommand represents the Ghoten a given run block will execute, plan
// or apply. Defaults to apply.
type TestCommand rune

// TestMode represents the plan mode that Ghoten will use for a given run
// block, normal or refresh-only. Defaults to normal.
type TestMode rune

const (
	// ApplyTestCommand causes the run block to execute a Ghoten apply
	// operation.
	ApplyTestCommand TestCommand = 0

	// PlanTestCommand causes the run block to execute a Ghoten plan
	// operation.
...
//
// Each run block represents a single Ghoten command to be executed and a set
// of validations to run after the command.
type TestRun struct {
	Name string

	// Command is the Ghoten command to execute.
	//
	// One of ['apply', 'plan'].
	Command TestCommand
...
type TestRunOptions struct {
	...
	// Refresh is analogous to the -refresh=false Ghoten plan option.
	Refresh bool

	// Replace is analogous to the -refresh=ADDRESS Ghoten plan option.
	Replace []hcl.Traversal

	// Target is analogous to the -target=ADDRESS Ghoten plan option.
				...
				default:
					if haveVersionArg {
						...
						diags = append(diags, &hcl.Diagnostic{
							...
							Detail:   fmt.Sprintf("Failed to parse module registry address: %s.\n\nGhoten assumed that you intended a module registry source address because you also set the argument \"version\", which applies only to registry modules.", err),
							Subject:  module.SourceDeclRange.Ptr(),
						})
					} else {
						diags = append(diags, &hcl.Diagnostic{
