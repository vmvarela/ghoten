...
import (
	...
	"github.com/vmvarela/ghoten/internal/experiments"
		...
		for _, required := range constraint.Required {
			if required.Prerelease() {
				prereleaseDiags = prereleaseDiags.Append(&hcl.Diagnostic{
					...
					Detail: fmt.Sprintf(
						"Prerelease version constraints are not supported: %s. Remove the prerelease information from the constraint. Prerelease versions of Ghoten will match constraints using their version core only.",
						required.String()),
		...
		if !constraint.Required.Check(tfversion.SemVer) {
			...
			case len(path) == 0:
				diags = diags.Append(&hcl.Diagnostic{
					Severity: hcl.DiagError,
					Summary:  "Unsupported Ghoten Core version",
					Detail: fmt.Sprintf(
						"This configuration does not support Ghoten version %s. To proceed, either choose another supported Ghoten version or update this version constraint. Version constraints are normally set for good reason, so updating the constraint may lead to other errors or unexpected behavior.",
						tfversion.String(),
					),
					Subject: constraint.DeclRange.Ptr(),
				})
			default:
				diags = diags.Append(&hcl.Diagnostic{
					...
					Summary:  "Unsupported Ghoten Core version",
					Detail: fmt.Sprintf(
						"Module %s (from %s) does not support Ghoten version %s. To proceed, either choose another supported Ghoten version or update this version constraint. Version constraints are normally set for good reason, so updating the constraint may lead to other errors or unexpected behavior.",
