		...
		if target, exists := c.Module.ManagedResources[i.StaticTo.String()]; exists {
			...
			if i.ProviderConfigRef != nil {
				...
				if i.ProviderConfigRef.Name != target.ProviderConfigRef.Name || i.ProviderConfigRef.Alias != target.ProviderConfigRef.Alias {
					...
					// This is bad as Ghoten now has different instructions
...
// This is mainly used for enabling warnings when Ghoten fails to resolve the implicitly generated provider.
...
func (c *Config) getProviderConfigTransformForTest(evalCtx *hcl.EvalContext) testConfigTransformFunc {
	return func(run *TestRun, file *TestFile) (func(), hcl.Diagnostics) {
		...
		// also specify a set of overrides that tell Ghoten exactly which
