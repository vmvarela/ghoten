...
// These are the "Report"-prefixed methods of Checks used by Ghoten Core
// to gradually signal the results of checks during a plan or apply operation.

// ReportCheckableObjects is the interface by which Ghoten Core should
...
func (c *State) ReportCheckableObjects(configAddr addrs.ConfigCheckable, objectAddrs addrs.Set[addrs.Checkable]) {
	...
	// so that even if we never hear from Ghoten Core again we'll still
...
// ReportCheckResult is the interface by which Ghoten Core should tell the
