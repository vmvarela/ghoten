...
type Change struct {
	...
	// An unordered set of paths into the old value which are marked as
	// ephemeral. Values at these paths must not be persisted in any
	// human-readable or stored output. This set is always empty for create.
	BeforeEphemeralPaths []*Path `protobuf:"bytes,7,rep,name=before_ephemeral_paths,json=beforeEphemeralPaths,proto3" json:"before_ephemeral_paths,omitempty"`
	// An unordered set of paths into the new value which are marked as
	// ephemeral. Values at these paths must not be persisted in any
	// human-readable or stored output. This set is always empty for delete.
	AfterEphemeralPaths []*Path `protobuf:"bytes,8,rep,name=after_ephemeral_paths,json=afterEphemeralPaths,proto3" json:"after_ephemeral_paths,omitempty"`
...
func (x *Change) GetBeforeEphemeralPaths() []*Path {
	if x != nil {
		return x.BeforeEphemeralPaths
	}
	return nil
}

func (x *Change) GetAfterEphemeralPaths() []*Path {
	if x != nil {
		return x.AfterEphemeralPaths
	}
	return nil
}
...
const file_planfile_proto_rawDesc = "" +
	...
	"\tworkspace\x18\x03 \x01(\tR\tworkspace\"\xc6\x03\n" +
	"\x06Change\x12&\n" +
	...
	"\x15after_sensitive_paths\x18\x04 \x03(\v2\f.tfplan.PathR\x13afterSensitivePaths\x12B\n" +
	"\x16before_ephemeral_paths\x18\a \x03(\v2\f.tfplan.PathR\x14beforeEphemeralPaths\x12@\n" +
	"\x15after_ephemeral_paths\x18\b \x03(\v2\f.tfplan.PathR\x13afterEphemeralPaths\x12/\n" +
	...
	"*FORGOT_BECAUSE_LIFECYCLE_DESTROY_IN_CONFIG\x10\x10B>Z<github.com/vmvarela/ghoten/internal/plans/internal/planprotob\x06proto3"
...
var file_planfile_proto_depIdxs = []int32{
	...
	12, // 13: tfplan.Change.before_ephemeral_paths:type_name -> tfplan.Path
	12, // 14: tfplan.Change.after_ephemeral_paths:type_name -> tfplan.Path
