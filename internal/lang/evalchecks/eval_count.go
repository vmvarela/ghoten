...
func EvaluateCountExpression(expr hcl.Expression, ctx EvaluateFunc, excludableAddr addrs.Targetable) (int, tfdiags.Diagnostics) {
	...
	if !countVal.IsKnown() {
		...
		diags = diags.Append(&hcl.Diagnostic{
			...
			Detail:   "The \"count\" value depends on resource attributes that cannot be determined until apply, so Ghoten cannot predict how many instances will be created.\n\n" + suggestion,
