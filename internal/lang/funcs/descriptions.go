...
// functions available within Ghoten. A function's description should point
...
var DescriptionList = map[string]descriptionEntry{
	...
	"sensitive": {
		Description:      "`sensitive` takes any value and returns a copy of it marked so that Ghoten will treat it as sensitive, with the same meaning and behavior as for [sensitive input variables](/language/values/variables#suppressing-values-in-cli-output).",
		ParamDescription: []string{""},
	},
	"setintersection": {
	...
	"textencodebase64": {
		Description:      "`textencodebase64` encodes the unicode characters in a given string using a specified character encoding, returning the result base64 encoded because Ghoten language strings are always sequences of unicode characters.",
		ParamDescription: []string{"", ""},
	},
	"timeadd": {
