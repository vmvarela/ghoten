...
// Ghoten, because we should always be tracking requests correctly.
...
func diagnosticsForWorkgraphErrorTracked(err error, tracker RequestTracker) tfdiags.Diagnostics {
	...
	case workgraph.ErrSelfDependency:
		...
		// Ghoten: we will get in here if Ghoten is tracking everything
		...
		for _, reqID := range err.RequestIDs {
			desc := "<unknown object> (failing to report this is a bug in Ghoten)"
	...
	case workgraph.ErrUnresolved:
		...
		diags = diags.Append(&hcl.Diagnostic{
			...
			Detail:   fmt.Sprintf("During configuration evaluation, %q was left unresolved. This is a bug in Ghoten.", reqName),
			Subject:  sourceRange,
		})
	default:
		...
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Error,
			"Evaluation failed",
			"Configuration evaluation failed for an unknown reason. This is a bug in Ghoten.",
		))
	}
	return diags
}
...
func diagnosticsForWorkgraphErrorUntracked(err error) tfdiags.Diagnostics {
	...
	case workgraph.ErrUnresolved:
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Error,
			"Evaluation failed",
			"An unexpected problem prevented complete evaluation of the configuration. This is a bug in Ghoten.",
		))
	case workgraph.ErrSelfDependency:
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Error,
			"Self-referential expressions",
			"The configuration contains expressions that form a dependency cycle. Unfortunately, a bug in Ghoten prevents reporting the affected expressions.",
		))
	default:
		// We should not get here because the two cases above cover everything
		// that package workgraph should return.
