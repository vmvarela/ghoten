...
import (
	...
	"github.com/vmvarela/ghoten/internal/modsdir"
	"github.com/vmvarela/ghoten/internal/registry"
	"github.com/vmvarela/ghoten/internal/registry/response"
...
//
// "registryClient" is the client for the Ghoten module registry protocol,
...
// fetched from an Ghoten module registry. This argument can be nil, in which
...
// Ghoten bug when failing to get the module and a user configuration error where they've specified a
		...
		func(ctx context.Context, req *configs.ModuleRequest) (*configs.Module, *version.Version, hcl.Diagnostics) {
			...
			ctx, span := tracing.Tracer().Start(ctx,
				...
				tracing.SpanAttributes(
					traceattrs.GhotenModuleCallName(req.Name),
					traceattrs.GhotenModuleSource(req.SourceAddr.String()),
				),
			)
			defer span.End()
			...
			if !recorded {
				...
				if err != nil && !os.IsNotExist(err) {
					...
					diags = diags.Append(&hcl.Diagnostic{
						...
						Detail: fmt.Sprintf(
							"Ghoten tried to remove %s in order to reinstall this module, but encountered an error: %s",
							instPath, err,
						),
					})
					return nil, nil, diags
...
func (i *ModuleInstaller) installRegistryModule(ctx context.Context, req *configs.ModuleRequest, key string, instPath string, addr addrs.ModuleSourceRegistry, manifest modsdir.Manifest, hooks ModuleInstallHooks, fetcher *getmodules.PackageFetcher) (*configs.Module, *version.Version, hcl.Diagnostics) {
	...
	ctx, span := tracing.Tracer().Start(ctx, "Install Registry Module", tracing.SpanAttributes(
		traceattrs.GhotenModuleCallName(req.Name),
		traceattrs.GhotenModuleSource(req.SourceAddr.String()),
		traceattrs.GhotenModuleVersion(req.VersionConstraint.Required.String()),
	))
	defer span.End()
		...
		if err != nil {
			if registry.IsModuleNotFound(err) {
				...
				if hostname == addrs.DefaultModuleRegistryHost {
					suggestion = "\n\nIf you believe this module is missing from the registry, please submit a issue on the Ghoten Registry https://github.com/opentofu/registry/issues/new/choose"
				}
	...
	if len(resp.Modules) < 1 {
		...
		diags = diags.Append(&hcl.Diagnostic{
			...
			Detail:   fmt.Sprintf("The registry at %s returned an invalid response when Ghoten requested available versions for module %q (%s:%d).", hostname, req.Name, req.CallRange.Filename, req.CallRange.Start.Line),
			Subject:  req.CallRange.Ptr(),
		})
		return nil, nil, diags
	}
	...
	for _, mv := range modMeta.Versions {
		...
		if err != nil {
			...
			diags = diags.Append(&hcl.Diagnostic{
				...
				Summary:  "Invalid response from remote module registry",
				Detail:   fmt.Sprintf("The registry at %s returned an invalid version string %q for module %q (%s:%d), which Ghoten ignored.", hostname, mv.Version, req.Name, req.CallRange.Filename, req.CallRange.Start.Line),
				Subject:  req.CallRange.Ptr(),
			})
			continue
		}
		...
		if v.Prerelease() != "" {
			...
			// syntax rather than the Ruby-style syntax Ghoten otherwise
			// uses. This should have been written to use
			// versions.MeetingConstraintsStringRuby instead, but changing it
			// now risks having Ghoten select a prerelease in more situations
			...
			//     https://github.com/vmvarela/ghoten/issues/2117
	...
	if _, exists := i.registryPackageSources[moduleAddr]; !exists {
		...
		span.SetAttributes(traceattrs.GhotenModuleSource(realAddr.String()))
	...
	if mod == nil {
		...
		if subDir != "" && isMissingSubDir {
			...
			diags = diags.Append(&hcl.Diagnostic{
				Severity: hcl.DiagError,
				Summary:  "Unreadable module directory",
				Detail:   fmt.Sprintf("The directory %s could not be read. This is a bug in Ghoten and should be reported.", modDir),
			})
		}
	} else if vDiags := mod.CheckCoreVersionRequirements(req.Path, req.SourceAddr); vDiags.HasErrors() {
		// If the core version requirements are not met, we drop any other
	...
	for _, step := range localRefs {
		...
		if !strings.HasPrefix(nextPath, prefix) { // ESCAPED!
			...
			if strings.HasPrefix(packageAddr, "/") || filepath.VolumeName(packageAddr) != "" {
				// It might be somewhat surprising that Ghoten treats
				...
				// about it.
				suggestion = "\n\nGhoten treats absolute filesystem paths as external modules which establish a new module package. To treat this directory as part of the same package as its caller, use a local path starting with either \"./\" or \"../\"."
			}
