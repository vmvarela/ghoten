...
// ApplyOpts are the various options that affect the details of how Ghoten
// will build a plan.
//
		...
		if forgetCount > 0 {
			...
			if !suppressError {
				diags = diags.Append(tfdiags.Sourceless(
					...
					`As requested, Ghoten has not deleted some remote objects that are no longer managed by this configuration. Those objects continue to exist in their remote system and so may continue to incur charges. Refer to the original plan for more information.
...
Note that the -target and -exclude options are not suitable for routine use, and are provided only for exceptional situations such as recovering from errors or mistakes, or when Ghoten specifically suggests to use it as part of an error message.`,
		))
	}
...
func (c *Context) mergePlanAndApplyVariables(config *configs.Config, plan *plans.Plan, opts *ApplyOpts) (InputValues, tfdiags.Diagnostics) {
	...
	for name, cfg := range config.Module.Variables {
		...
		// which case Ghoten will once again use the default value from the
