...
`
	...
	// operations we typically fail more deeply in Ghoten Core, with
		...
		t.Run(testName, func(t *testing.T) {
			...
			wantDiags = wantDiags.Append(
				tfdiags.Sourceless(
					...
					"This configuration requires built-in provider terraform.io/builtin/nonexist, but that provider isn't available in this Ghoten version.",
				),
				tfdiags.Sourceless(
					tfdiags.Error,
					"Missing required provider",
				...
				tfdiags.Sourceless(
					...
					`This configuration requires provisioner plugin "nonexist", which isn't available. If you're intending to use an external provisioner plugin, you must install it manually into one of the plugin search directories before running Ghoten.`,
				),
			)
...
func TestContext_contextValuesPropagation(t *testing.T) {
	...
	probe.ExpectReportsFrom(t,
		"github.com/vmvarela/ghoten/internal/ghoten.(*MockProvider).GetProviderSchema",
		"github.com/vmvarela/ghoten/internal/ghoten.(*MockProvider).ValidateProviderConfig",
		"github.com/vmvarela/ghoten/internal/ghoten.(*MockProvider).ValidateDataResourceConfig",
		"github.com/vmvarela/ghoten/internal/ghoten.(*MockProvider).ValidateResourceConfig",
		"github.com/vmvarela/ghoten/internal/ghoten.(*MockProvider).ConfigureProvider",
		"github.com/vmvarela/ghoten/internal/ghoten.(*MockProvider).ReadDataSource",
		"github.com/vmvarela/ghoten/internal/ghoten.(*MockProvider).PlanResourceChange",
		"github.com/vmvarela/ghoten/internal/ghoten.(*MockProvider).ApplyResourceChange",
		"github.com/vmvarela/ghoten/internal/ghoten.(*MockProvider).Close",
	)
}
