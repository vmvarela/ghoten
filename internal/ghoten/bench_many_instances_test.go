...
//
// Ghoten's current design is aimed to support tens of instances as the typical case
...
// since we know that some in our community use Ghoten in that way and although it
...
// "unreasonable" number of instances (per Ghoten's current design assumptions),
// or whenever we're intentionally attempting to change something in Ghoten to
// improve performance.
//
// The existence of these benchmarks does not represent a commitment to support
// using Ghoten with thousands of resource instances in the same configuration.
...
func BenchmarkManyResourceInstances(b *testing.B) {
	...
	tofuCtx := testContext2(b, &ContextOpts{
		...
		// Ghoten's direct overheads and not by the artificial concurrency
		// limit.
