...
// callback. This allows you to modify the behavior of Ghoten at runtime.
...
const (
	...
	// and the action that Ghoten was about to take is cancelled.
	HookActionHalt
)

// Hook is the interface that must be implemented to hook into various
// parts of Ghoten, allowing you to inspect or change behavior at runtime.
//
// There are MANY hook points into Ghoten. If you only want to implement
...
type Hook interface {
	...
	// Stopping is called if an external signal requests that Ghoten
	// gracefully abort an operation in progress.
	//
	// This notification might suggest that the user wants Ghoten to exit
	// ASAP and in that case it's possible that if Ghoten runs for too much
	...
	//
	// This hook cannot control whether Ghoten continues, because the
