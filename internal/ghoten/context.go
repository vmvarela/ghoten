...
// into Ghoten in various ways. This must be provided by the Context
// initializer.
type ContextMeta struct {
	Env string // Env is the state environment

	// OriginalWorkingDir is the working directory where the Ghoten CLI
...
// Context represents all the context that Ghoten needs in order to
...
func NewContext(opts *ContextOpts) (*Context, tfdiags.Diagnostics) {
	...
	log.Printf("[TRACE] ghoten.NewContext: starting")
	...
	log.Printf("[TRACE] ghoten.NewContext: complete")
...
//
// Currently this function checks whether the current Ghoten CLI version
...
func (c *Context) checkConfigDependencies(config *configs.Config) tfdiags.Diagnostics {
	...
	// This checks the Ghoten CLI version constraints specified in all of
	// the modules.
		...
		if !c.plugins.HasProvider(providerAddr) {
			if !providerAddr.IsBuiltIn() {
				...
				diags = diags.Append(tfdiags.Sourceless(
					...
					"Missing required provider",
					fmt.Sprintf(
						"This configuration requires built-in provider %s, but that provider isn't available in this Ghoten version.",
						providerAddr,
					),
				))
			}
		...
		for _, rc := range modCfg.Module.ManagedResources {
			...
			for _, pc := range rc.Managed.Provisioners {
				if !c.plugins.HasProvisioner(pc.Type) {
					...
					// the caller of ghoten.NewContext should've already
					...
					diags = diags.Append(tfdiags.Sourceless(
						...
						fmt.Sprintf(
							"This configuration requires provisioner plugin %q, which isn't available. If you're intending to use an external provisioner plugin, you must install it manually into one of the plugin search directories before running Ghoten.",
							pc.Type,
						),
					))
				}
