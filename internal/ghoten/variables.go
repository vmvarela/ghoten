...
// provided by the external caller into a function like ghoten.Context.Plan.
//
...
// that wasn't set. Those adjustments will be handled by Ghoten Core itself
// as part of performing the requested operation.
//
// A Ghoten Core caller must provide an InputValue object for each of the
...
//
// Ghoten Core also internally uses InputValue to represent the raw value
...
type InputValue struct {
	...
	// came from, which Ghoten Core uses to tailor some of its error
...
func (v *InputValue) GoString() string {
	if (v.SourceRange != tfdiags.SourceRange{}) {
		return fmt.Sprintf("&ghoten.InputValue{Value: %#v, SourceType: %#v, SourceRange: %#v}", v.Value, v.SourceType, v.SourceRange)
	} else {
		return fmt.Sprintf("&ghoten.InputValue{Value: %#v, SourceType: %#v}", v.Value, v.SourceType)
	}
}
	...
	for name := range vcs {
		...
		if !isSet {
			...
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				"Unassigned variable",
				fmt.Sprintf("The input variable %q has not been assigned a value. This is a bug in Ghoten; please report it in a GitHub issue.", name),
			))
			continue
		}
	}
