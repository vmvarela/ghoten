...
// for Ghoten. Usage of schema is recommended over attempting to write
...
type Schema struct {
	...
	// secret fields. Future versions of Ghoten may encrypt these
	// values.
	Sensitive bool
}
...
func (s *Schema) finalizeDiff(d *ghoten.ResourceAttrDiff, customized bool) *ghoten.ResourceAttrDiff {
	if d == nil {
		return d
	}
...
func (m schemaMap) Data(
	...
	d *ghoten.InstanceDiff) (*ResourceData, error) {
	return &ResourceData{
		schema:       m,
		state:        s,
		diff:         d,
...
func (m schemaMap) Diff(
	...
	meta interface{},
	handleRequiresNew bool) (*ghoten.InstanceDiff, error) {
	result := new(ghoten.InstanceDiff)
	result.Attributes = make(map[string]*ghoten.ResourceAttrDiff)
	...
	if handleRequiresNew {
		...
		if result.RequiresNew() {
			// Create the new diff
			result2 := new(ghoten.InstanceDiff)
			result2.Attributes = make(map[string]*ghoten.ResourceAttrDiff)
...
// Input implements the ghoten.ResourceProvider method by asking
...
func (m schemaMap) Validate(c *ghoten.ResourceConfig) ([]string, []error) {
...
func (m schemaMap) diff(
	...
	unsuppressedDiff := new(ghoten.InstanceDiff)
	unsuppressedDiff.Attributes = make(map[string]*ghoten.ResourceAttrDiff)
...
func (m schemaMap) diffList(
	...
	if computedList {
		diff.Attributes[k+".#"] = &ghoten.ResourceAttrDiff{
			Old:         oldStr,
			NewComputed: true,
...
func (m schemaMap) inputString(
	...
	result, err := input.Input(context.Background(), &ghoten.InputOpts{
		Id:          k,
		Query:       k,
...
func (m schemaMap) validate(
	k string,
	schema *Schema,
	c *ghoten.ResourceConfig) ([]string, []error) {
	raw, ok := c.Get(k)
