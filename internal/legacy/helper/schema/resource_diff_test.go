...
func testDiffCases(t *testing.T, oldPrefix string, oldOffset int, computed bool) []resourceDiffTestCase {
	return []resourceDiffTestCase{
		...
		resourceDiffTestCase{
			...
			Diff: &ghoten.InstanceDiff{
				Attributes: map[string]*ghoten.ResourceAttrDiff{
					"foo.1996459178": &ghoten.ResourceAttrDiff{
						Old:        "bar",
						New:        "",
						NewRemoved: true,
					},
					"foo.2015626392": &ghoten.ResourceAttrDiff{
						Old: "",
						New: "baz",
					},
				},
			...
			Expected: &ghoten.InstanceDiff{
				Attributes: func() map[string]*ghoten.ResourceAttrDiff {
					result := map[string]*ghoten.ResourceAttrDiff{}
					if computed {
						result["foo.#"] = &ghoten.ResourceAttrDiff{
							Old:         "1",
							New:         "",
							NewComputed: true,
						}
					} else {
						result["foo.1494962232"] = &ghoten.ResourceAttrDiff{
							Old: "",
							New: "qux",
						}
						result["foo.1996459178"] = &ghoten.ResourceAttrDiff{
							Old:        "bar",
							New:        "",
							NewRemoved: true,
						}
		...
		resourceDiffTestCase{
			...
			Expected: &ghoten.InstanceDiff{
				Attributes: func() map[string]*ghoten.ResourceAttrDiff {
					result := make(map[string]*ghoten.ResourceAttrDiff)
					if computed {
						result["foo.#"] = &ghoten.ResourceAttrDiff{
							Old:         "1",
							New:         "",
						...
						result["foo.0"] = &ghoten.ResourceAttrDiff{
							Old: "bar",
							New: "qux",
						}
					}
		...
		resourceDiffTestCase{
			...
			Diff: &ghoten.InstanceDiff{
				Attributes: map[string]*ghoten.ResourceAttrDiff{
					"foo.bar": &ghoten.ResourceAttrDiff{
						Old: "baz",
						New: "qux",
					},
				},
			...
			Expected: &ghoten.InstanceDiff{
				Attributes: func() map[string]*ghoten.ResourceAttrDiff {
					...
					if computed {
						...
						result["foo.bar"] = &ghoten.ResourceAttrDiff{
							Old:        "baz",
							New:        "",
							NewRemoved: true,
						}
		...
		resourceDiffTestCase{
			...
			Diff: &ghoten.InstanceDiff{
				Attributes: map[string]*ghoten.ResourceAttrDiff{
					"top.4.foo": &ghoten.ResourceAttrDiff{
						Old: "",
						New: "1",
					},
					"top.4.bar": &ghoten.ResourceAttrDiff{
						Old: "",
						New: "3",
					},
					"top.24.foo": &ghoten.ResourceAttrDiff{
						Old: "",
						New: "12",
					},
					"top.24.bar": &ghoten.ResourceAttrDiff{
						Old: "",
						New: "12",
					},
				},
			...
			Expected: &ghoten.InstanceDiff{
				Attributes: func() map[string]*ghoten.ResourceAttrDiff {
					result := make(map[string]*ghoten.ResourceAttrDiff)
					if computed {
						result["top.#"] = &ghoten.ResourceAttrDiff{
							Old:         "2",
							New:         "",
							NewComputed: true,
						}
						...
						result["top.5.foo"] = &ghoten.ResourceAttrDiff{
							Old: "",
							New: "1",
						}
						result["top.5.bar"] = &ghoten.ResourceAttrDiff{
							Old: "",
							New: "4",
						}
						result["top.25.foo"] = &ghoten.ResourceAttrDiff{
							Old: "",
							New: "13",
						}
						result["top.25.bar"] = &ghoten.ResourceAttrDiff{
							Old: "",
							New: "12",
						}
						result["top.43.foo"] = &ghoten.ResourceAttrDiff{
							Old: "",
							New: "21",
						}
						result["top.43.bar"] = &ghoten.ResourceAttrDiff{
							Old: "",
							New: "22",
						}
					}
		...
		resourceDiffTestCase{
			...
			Config:   testConfig(t, map[string]interface{}{}),
			Diff:     &ghoten.InstanceDiff{Attributes: map[string]*ghoten.ResourceAttrDiff{}},
			Key:      "foo",
			NewValue: "baz",
			Expected: &ghoten.InstanceDiff{
				Attributes: map[string]*ghoten.ResourceAttrDiff{
		...
		resourceDiffTestCase{
			...
			Expected: &ghoten.InstanceDiff{
				Attributes: func() map[string]*ghoten.ResourceAttrDiff {
					if computed {
						return map[string]*ghoten.ResourceAttrDiff{
							"foo": &ghoten.ResourceAttrDiff{
								NewComputed: computed,
							},
						}
...
func TestForceNew(t *testing.T) {
	cases := []resourceDiffTestCase{
		...
		resourceDiffTestCase{
			...
			Diff: &ghoten.InstanceDiff{
				Attributes: map[string]*ghoten.ResourceAttrDiff{
					"foo.0.bar": &ghoten.ResourceAttrDiff{
						Old: "abc",
						New: "abcdefg",
					},
					"foo.0.baz": &ghoten.ResourceAttrDiff{
						Old: "xyz",
						New: "changed",
					},
				},
...
func TestClear(t *testing.T) {
	cases := []resourceDiffTestCase{
		resourceDiffTestCase{
			...
			Key:      "foo",
			Expected: &ghoten.InstanceDiff{Attributes: map[string]*ghoten.ResourceAttrDiff{}},
		},
		resourceDiffTestCase{
			Name: "non-computed key, should error",
			Schema: map[string]*Schema{
...
func TestGetChangedKeysPrefix(t *testing.T) {
	cases := []resourceDiffTestCase{
		...
		resourceDiffTestCase{
			...
			Diff: &ghoten.InstanceDiff{
				Attributes: map[string]*ghoten.ResourceAttrDiff{
					"testfield": &ghoten.ResourceAttrDiff{
						Old: "blablah",
						New: "modified",
					},
					"foo.0.bar": &ghoten.ResourceAttrDiff{
