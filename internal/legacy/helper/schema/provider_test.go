...
func TestProvider_impl(t *testing.T) {
	var _ ghoten.ResourceProvider = new(Provider)
}
...
func TestProviderGetSchema(t *testing.T) {
	...
	got, err := p.GetSchema(&ghoten.ProviderSchemaRequest{
...
func TestProviderResources(t *testing.T) {
	cases := []struct {
...

		{
			...
			Result: []ghoten.ResourceType{
				ghoten.ResourceType{Name: "bar", SchemaAvailable: true},
				ghoten.ResourceType{Name: "foo", SchemaAvailable: true},
			},
		},

		{
			...
			Result: []ghoten.ResourceType{
				ghoten.ResourceType{Name: "bar", Importable: true, SchemaAvailable: true},
				ghoten.ResourceType{Name: "baz", SchemaAvailable: true},
				ghoten.ResourceType{Name: "foo", SchemaAvailable: true},
			},
		},
	}
...
func TestProviderDataSources(t *testing.T) {
	cases := []struct {
...

		{
			...
			Result: []ghoten.DataSource{
				ghoten.DataSource{Name: "bar", SchemaAvailable: true},
				ghoten.DataSource{Name: "foo", SchemaAvailable: true},
			},
		},
	}
...
func TestProviderDiff_legacyTimeoutType(t *testing.T) {
	...
	ic := ghoten.NewResourceConfigRaw(invalidCfg)
	_, err := p.Diff(
		&ghoten.InstanceInfo{
			Type: "blah",
		},
...
func TestProviderImportState_default(t *testing.T) {
	...
	states, err := p.ImportState(&ghoten.InstanceInfo{
		Type: "foo",
	}, "bar")
	if err != nil {
		t.Fatalf("err: %s", err)
...
func TestProviderImportState_setsId(t *testing.T) {
	...
	_, err := p.ImportState(&ghoten.InstanceInfo{
		Type: "foo",
	}, "bar")
	if err != nil {
		t.Fatalf("err: %s", err)
