...
// Provider represents a resource provider in Ghoten, and properly
...
// Stop implementation of ghoten.ResourceProvider interface.
...
// GetSchema implementation of ghoten.ResourceProvider interface
func (p *Provider) GetSchema(req *ghoten.ProviderSchemaRequest) (*ghoten.ProviderSchema, error) {
...
// Input implementation of ghoten.ResourceProvider interface.
func (p *Provider) Input(
	input ghoten.UIInput,
	c *ghoten.ResourceConfig) (*ghoten.ResourceConfig, error) {
	return schemaMap(p.Schema).Input(input, c)
}

// Validate implementation of ghoten.ResourceProvider interface.
func (p *Provider) Validate(c *ghoten.ResourceConfig) ([]string, []error) {
...
// ValidateResource implementation of ghoten.ResourceProvider interface.
func (p *Provider) ValidateResource(
	t string, c *ghoten.ResourceConfig) ([]string, []error) {
...
// Configure implementation of ghoten.ResourceProvider interface.
func (p *Provider) Configure(c *ghoten.ResourceConfig) error {
	// No configuration
...
// Apply implementation of ghoten.ResourceProvider interface.
func (p *Provider) Apply(
	info *ghoten.InstanceInfo,
	s *ghoten.InstanceState,
	d *ghoten.InstanceDiff) (*ghoten.InstanceState, error) {
...
// Diff implementation of ghoten.ResourceProvider interface.
func (p *Provider) Diff(
	info *ghoten.InstanceInfo,
	s *ghoten.InstanceState,
	c *ghoten.ResourceConfig) (*ghoten.InstanceDiff, error) {
	r, ok := p.ResourcesMap[info.Type]
	if !ok {
		return nil, fmt.Errorf("unknown resource type: %s", info.Type)
	}
...
// Refresh implementation of ghoten.ResourceProvider interface.
func (p *Provider) Refresh(
	info *ghoten.InstanceInfo,
	s *ghoten.InstanceState) (*ghoten.InstanceState, error) {
	r, ok := p.ResourcesMap[info.Type]
	if !ok {
		return nil, fmt.Errorf("unknown resource type: %s", info.Type)
	}
...
// Resources implementation of ghoten.ResourceProvider interface.
func (p *Provider) Resources() []ghoten.ResourceType {
	...
	result := make([]ghoten.ResourceType, 0, len(keys))
	for _, k := range keys {
		...
		result = append(result, ghoten.ResourceType{
			Name:       k,
...
func (p *Provider) ImportState(
	info *ghoten.InstanceInfo,
	id string) ([]*ghoten.InstanceState, error) {
	// Find the resource
	r, ok := p.ResourcesMap[info.Type]
	if !ok {
		return nil, fmt.Errorf("unknown resource type: %s", info.Type)
	...
	states := make([]*ghoten.InstanceState, len(results))
...
// ValidateDataSource implementation of ghoten.ResourceProvider interface.
...
// ReadDataDiff implementation of ghoten.ResourceProvider interface.
...
// RefreshData implementation of ghoten.ResourceProvider interface.
...
// DataSources implementation of ghoten.ResourceProvider interface.
func (p *Provider) DataSources() []ghoten.DataSource {
	...
	result := make([]ghoten.DataSource, 0, len(keys))
	for _, k := range keys {
		result = append(result, ghoten.DataSource{
			Name: k,
