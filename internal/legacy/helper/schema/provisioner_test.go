...
func TestProvisioner_impl(t *testing.T) {
	var _ ghoten.ResourceProvisioner = new(Provisioner)
}
	...
	cases := []struct {
		...
		},
		{
			...
			P: &Provisioner{
				Schema:    nil,
				ApplyFunc: noopApply,
				ValidateFunc: func(*ghoten.ResourceConfig) (ws []string, errors []error) {
	...
	for i, tc := range cases {
		t.Run(fmt.Sprintf("%d-%s", i, tc.Name), func(t *testing.T) {
			...
			state := &ghoten.InstanceState{
				Ephemeral: ghoten.EphemeralState{
					ConnInfo: tc.Conn,
				},
			}
...
func TestProvisionerApply_nilState(t *testing.T) {
	...
	c := ghoten.NewResourceConfigRaw(conf)
