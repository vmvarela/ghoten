...
type Provisioner struct {
	...
	ValidateFunc func(*ghoten.ResourceConfig) ([]string, []error)
...
var (
	...
	// This returns a ghoten.UIOutput. Guaranteed to never be nil.
...
// Stop implementation of ghoten.ResourceProvisioner interface.
...
// GetConfigSchema implementation of ghoten.ResourceProvisioner interface.
func (p *Provisioner) GetConfigSchema() (*configschema.Block, error) {
	return schemaMap(p.Schema).CoreConfigSchema(), nil
}

// Apply implementation of ghoten.ResourceProvisioner interface.
func (p *Provisioner) Apply(
...

	{
		...
		// ghoten.ResourceConfig so that we can use that type to more
		...
		c := ghoten.NewResourceConfigRaw(raw)
		sm := schemaMap(p.ConnSchema)
...
// Validate implements the ghoten.ResourceProvisioner interface.
func (p *Provisioner) Validate(c *ghoten.ResourceConfig) (ws []string, es []error) {
