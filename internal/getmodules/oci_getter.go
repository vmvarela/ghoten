...
// manifest describing an Ghoten module package.
...
//
// Because this implementation lives inside Ghoten rather than upstream
// go-getter, it intentionally focuses only on the subset of go-getter
// functionality that Ghoten's module installer uses. If we do someday
...
func (g *ociDistributionGetter) GetFile(string, *url.URL) error {
	// With how Ghoten uses go-getter we can only get in here if
...
func (g *ociDistributionGetter) ClientMode(*url.URL) (getter.ClientMode, error) {
	...
	// Ghoten, because Ghoten _always_ asks for
...
func (g *ociDistributionGetter) resolveRepositoryRef(url *url.URL) (*orasRegistry.Reference, error) {
	...
	// The Ghoten module installer has already stripped off any
...
func (g *ociDistributionGetter) resolveManifestDescriptor(ctx context.Context, ref *orasRegistry.Reference, query url.Values, store OCIRepositoryStore) (desc ociv1.Descriptor, err error) {
	ctx, span := tracing.Tracer().Start(
		...
		tracing.SpanAttributes(
			traceattrs.GhotenOCIRegistryDomain(ref.Registry),
			traceattrs.GhotenOCIRepositoryName(ref.Repository),
		),
	)
	defer span.End()
	...
	if wantTag != "" {
		...
		span.SetAttributes(
			traceattrs.GhotenOCIReferenceTag(wantTag),
		)
		...
		span.SetAttributes(
			traceattrs.GhotenOCIReferenceDigest(wantDigest.String()),
		)
	...
	// We always expect ArtifactType to be set to our Ghoten-specific type,
...
func selectOCILayerBlob(descs []ociv1.Descriptor) (ociv1.Descriptor, error) {
	...
	for _, desc := range descs {
		if _, ok := goGetterDecompressorMediaTypes[desc.MediaType]; ok {
			...
			// supported media types so that future versions of Ghoten
	...
	if len(foundBlobs) == 0 {
		if foundWrongMediaTypeBlobs > 0 {
			return ociv1.Descriptor{}, fmt.Errorf("image manifest contains no layers of types supported as module packages by Ghoten, but has other unsupported formats; this OCI artifact might be intended for a different version of Ghoten")
		}
		return ociv1.Descriptor{}, fmt.Errorf("image manifest contains no layers of types supported as module packages by Ghoten")
	}
...
func fetchOCIBlobToTemporaryFile(ctx context.Context, desc ociv1.Descriptor, store orasContent.Fetcher) (tempFile string, err error) {
	ctx, span := tracing.Tracer().Start(
		...
		tracing.SpanAttributes(
			traceattrs.GhotenOCIBlobDigest(desc.Digest.String()),
			traceattrs.GhotenOCIBlobMediaType(desc.MediaType),
			traceattrs.GhotenOCIBlobSize(desc.Size),
		),
	)
	defer span.End()
