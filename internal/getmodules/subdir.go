...
func SplitPackageSubdir(given string) (packageAddr, subDir string) {
	...
	//
	// However, note that in Ghoten we _always_ split off the subdirectory
	// portion and handle it within Ghoten-level code, _never_ passing
	// a subdirectory portion down into go-getter's own Get function, because
	// Ghoten's ability to refer between local paths inside the same
	// package depends on Ghoten itself always being aware of where the
