...
// exporter to use
// If this environment variable is set to "otlp" when running Ghoten CLI
...
// trace parent to use.
// If this environment variable is set when running Ghoten CLI
...
const DefaultServiceName = "Ghoten CLI"
...
//
// By default, we don't export telemetry information at all, since Ghoten is
// a CLI tool, and so we don't assume we're running in an environment with
// a telemetry collector available.
//
// However, for those running Ghoten in automation we allow setting
...
// protocol buffers and gRPC, which Ghoten would depend on for other reasons
// anyway.
//
