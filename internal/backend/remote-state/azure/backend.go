...
import (
	...
	"github.com/vmvarela/ghoten/internal/backend/remote-state/azure/auth"
	...
	s := &schema.Backend{
		Schema: map[string]*schema.Schema{
			...
			"msi_endpoint": {
				...
				Deprecated:  "This configuration is now managed in a dependent library, not directly by Ghoten. Please use the `MSI_ENDPOINT` environment variable to set the Managed Service Identity endpoint.",
			},
			...
			"use_azuread_auth": {
				Type:        schema.TypeBool,
				Optional:    true,
				Description: "Should Ghoten use AzureAD Authentication to access the Blob?",
