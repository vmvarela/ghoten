...
type Remote struct {
	...
	// new Ghoten context. Many of these will be overridden or merged by
	// Operation. See Operation for more details.
	ContextOpts *ghoten.ContextOpts
	...
	// local Ghoten version matches the remote workspace's configured
...
func (b *Remote) Configure(ctx context.Context, obj cty.Value) tfdiags.Diagnostics {
	...
	if val := obj.GetAttr("hostname"); !val.IsNull() && val.AsString() != "" {
		...
		diags = diags.Append(tfdiags.Sourceless(
			...
			`Ghoten does not provide a default "hostname" attribute, so it must be set to the hostname of the remote backend.`,
		))
	...
	// Historical note: in Ghoten's predecessor project there was an
	...
	// itself to be compatible with. Since Ghoten's version numbers have
	...
	// Ghoten's releases in mind) we just skip that here and let the
	...
	cfg.Headers.Set("User-Agent", httpclient.GhotenUserAgent(tfversion.String()))
...
func (b *Remote) StateMgr(ctx context.Context, name string) (statemgr.Full, error) {
	...
	if err == tfe.ErrResourceNotFound {
		...
		// We only set the Ghoten Version for the new workspace if this is
	...
	if !b.ignoreVersionConflict {
		...
		if wsv != tfversion.String() && wsv != "latest" {
			return nil, fmt.Errorf("Remote workspace Ghoten version %q does not match local Ghoten version %q", workspace.TerraformVersion, tfversion.String())
		}
	}
	...
	client := &remoteClient{
		...
		// This is optionally set during Ghoten Enterprise runs.
...
func (b *Remote) Operation(ctx context.Context, op *backend.Operation) (*backend.RunningOperation, error) {
	...
	// Ghoten remote version conflicts are not a concern for operations. We
	...
	//   happen in the Terraform Cloud worker, in which case the Ghoten
...
func (b *Remote) cancel(cancelCtx context.Context, op *backend.Operation, r *tfe.Run) error {
	if r.Actions.IsCancelable {
		...
		if !op.AutoApprove {
			v, err := op.UIIn.Input(cancelCtx, &ghoten.InputOpts{
				Id:          "cancel",
...
// the local Ghoten version matches the remote workspace's configured
// Ghoten version. This should be called by commands where this check is
...
// VerifyWorkspaceTerraformVersion compares the local Ghoten version against
// the workspace's configured Ghoten version. If they are equal, this means
...
func (b *Remote) VerifyWorkspaceTerraformVersion(workspaceName string) tfdiags.Diagnostics {
	...
	// cannot reasonably determine what the intended Ghoten version is, so
	// we'll skip version verification.
	if workspace.TerraformVersion == "latest" {
		return nil
	}

	// If the workspace has remote operations disabled, the remote Ghoten
	...
	if err != nil {
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Error,
			"Error looking up workspace",
			fmt.Sprintf("Invalid Ghoten version: %s", err),
		))
		return diags
	}
	...
	if tfversion.SemVer.LessThan(v014) || remoteVersion.LessThan(v014) {
		// Versions of Ghoten prior to 0.14.0 will refuse to load state files
		// written by a newer version of Ghoten, even if it is only a patch
	...
	if tfversion.SemVer.GreaterThanOrEqual(v014) && remoteVersion.GreaterThanOrEqual(v014) {
		// Versions of Ghoten after 0.14.0 should be compatible with each
		...
		// - Any new Ghoten state version will require at least minor patch
	...
	suggestion := " If you're sure you want to upgrade the state, you can force Ghoten to continue using the -ignore-remote-version flag. This may result in an unusable workspace."
	if b.ignoreVersionConflict {
		suggestion = ""
	}
	diags = diags.Append(tfdiags.Sourceless(
		severity,
		"Ghoten version mismatch",
		fmt.Sprintf(
			"The local Ghoten version (%s) does not match the configured version for remote workspace %s/%s (%s).%s",
			tfversion.String(),
			b.organization,
			workspace.Name,
...
Ghoten to prevent data loss! Trying to restore the connection...
[reset]
`
