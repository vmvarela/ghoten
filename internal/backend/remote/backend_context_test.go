...
func TestRemoteVariablesDoNotOverride(t *testing.T) {
	...
	tests := map[string]struct {
		...
		expectedVariables ghoten.InputValues
	}{
		"no local variables": {
		...
		"single conflicting local variable": {
			...
			ghoten.InputValues{
				...
				varName3: &ghoten.InputValue{
					Value:      cty.StringVal(varValue3),
					SourceType: ghoten.ValueFromNamedFile,
					SourceRange: tfdiags.SourceRange{
