...
func (b *Remote) LocalRun(ctx context.Context, op *backend.Operation) (*backend.LocalRun, statemgr.Full, tfdiags.Diagnostics) {
	...
	log.Printf("[TRACE] backend/remote: finished building ghoten.Context")
...
func stubAllVariables(vv map[string]backend.UnparsedVariableValue, decls map[string]*configs.Variable) ghoten.InputValues {
	ret := make(ghoten.InputValues, len(decls))
...
// the Ghoten Core backend representation of variables.
...
func (v *remoteStoredVariableValue) ParseVariableValue(mode configs.VariableParsingMode) (*ghoten.InputValue, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
	var val cty.Value
	...
	case v.definition.HCL:
		...
		// file because that is how it would get passed to Ghoten CLI for
