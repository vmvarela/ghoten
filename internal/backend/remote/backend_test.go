...
func TestRemote_StateMgr_versionCheck(t *testing.T) {
	...
	// Update the mock remote workspace Ghoten version to match the local
	// Terraform version
	...
	// This should fail
	want := `Remote workspace Ghoten version "0.13.5" does not match local Ghoten version "0.14.0"`
...
func TestRemote_VerifyWorkspaceTerraformVersion(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("local %s, remote %s", tc.local, tc.remote), func(t *testing.T) {
			...
			// Update the mock remote workspace Ghoten version to the
			...
			if tc.wantErr {
				...
				if got := diags.Err().Error(); !strings.Contains(got, "Ghoten version mismatch") {
...
func TestRemote_VerifyWorkspaceTerraformVersion_workspaceErrors(t *testing.T) {
	...
	// Update the mock remote workspace Ghoten version to an invalid version
	if _, err := b.client.Workspaces.Update(
		context.Background(),
		b.organization,
		b.workspace,
	...
	if got := diags.Err().Error(); !strings.Contains(got, "Error looking up workspace: Invalid Ghoten version") {
		t.Fatalf("unexpected error: %s", got)
	}
}
...
func TestRemote_VerifyWorkspaceTerraformVersion_ignoreFlagSet(t *testing.T) {
	...
	if got, want := diags[0].Description().Summary, "Ghoten version mismatch"; got != want {
		t.Errorf("wrong summary: got %s, want %s", got, want)
	}
	wantDetail := "The local Ghoten version (0.14.0) does not match the configured version for remote workspace hashicorp/prod (0.13.5)."
