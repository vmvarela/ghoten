...
func (b *Local) opApply(
	...
	if op.PlanFile == nil {
		...
		diags = diags.Append(moreDiags)
		if moreDiags.HasErrors() {
			// If Ghoten Core generated a partial plan despite the errors
			// then we'll make the best effort to render it. Ghoten Core
		...
		if mustConfirm {
			...
			case plans.DestroyMode:
				...
				desc = "Ghoten will destroy all your managed infrastructure, as shown above.\n" +
					"There is no undo. Only 'yes' will be accepted to confirm."
			case plans.RefreshOnlyMode:
				if op.Workspace != "default" {
					query = "Would you like to update the Ghoten state for \"" + op.Workspace + "\" to reflect these detected changes?"
				} else {
					query = "Would you like to update the Ghoten state to reflect these detected changes?"
				}
				desc = "Ghoten will write these changes to the state without modifying any real infrastructure.\n" +
					"There is no undo. Only 'yes' will be accepted to confirm."
			default:
				if op.Workspace != "default" {
				...
				desc = "Ghoten will perform the actions described above.\n" +
			...
			v, err := op.UIIn.Input(stopCtx, &ghoten.InputOpts{
				Id:          "approve",
				Query:       "\n" + query,
				Description: desc,
			})
		...
		if plan.Errored {
			diags = diags.Append(tfdiags.Sourceless(
				...
				"Ghoten encountered an error when generating this plan, so it cannot be applied.",
			))
			op.ReportResult(runningOp, diags)
			return
		}
...
const stateWriteBackedUpError = `The error shown above has prevented Ghoten from writing the updated state to the configured backend. To allow for recovery, the state has been written to the file "errored.tfstate" in the current working directory.
...
const stateWriteConsoleFallbackError = `The errors shown above prevented Ghoten from writing the updated state to
...
A catastrophic error has prevented Ghoten from persisting the state file or creating a backup. Unfortunately this means that the record of any resources created during this apply has been lost, and such resources may exist outside of Ghoten's management.
