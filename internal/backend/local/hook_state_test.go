...
func TestStateHook_impl(t *testing.T) {
	var _ ghoten.Hook = new(StateHook)
}
...
func TestStateHook(t *testing.T) {
	...
	var hook ghoten.Hook = &StateHook{
		StateMgr: is,
	}
	...
	if action != ghoten.HookActionContinue {
...
func TestStateHookStopping(t *testing.T) {
	...
	if got, want := action, ghoten.HookActionContinue; got != want {
...
func (sm *testPersistentState) PersistState(_ context.Context, schemas *ghoten.Schemas) error {
	if schemas == nil {
...
func (sm *testPersistentStateThatRefusesToPersist) PersistState(_ context.Context, schemas *ghoten.Schemas) error {
	if schemas == nil {
		return fmt.Errorf("no schemas")
	}
	sm.CallLog = append(sm.CallLog, "PersistState")
