...
func (b *Local) localRun(ctx context.Context, op *backend.Operation) (*backend.LocalRun, *configload.Snapshot, statemgr.Full, tfdiags.Diagnostics) {
	...
	var coreOpts ghoten.ContextOpts
	...
	if op.PlanFile.IsCloud() {
		diags = diags.Append(fmt.Errorf("error: using a saved cloud plan when executing Ghoten locally is not supported"))
		return nil, nil, nil, diags
	}
...
func (b *Local) localRunDirect(ctx context.Context, op *backend.Operation, run *backend.LocalRun, coreOpts *ghoten.ContextOpts, s statemgr.Full) (*backend.LocalRun, *configload.Snapshot, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
	...
	if errs := config.VerifyDependencySelections(op.DependencyLocks); len(errs) > 0 {
		...
		case op.DependencyLocks == nil:
			...
			suggestion = "This run has no dependency lock information provided at all, which is a bug in Ghoten; please report it!"
	...
	run.ApplyOpts = &ghoten.ApplyOpts{
	...
	tfCtx, moreDiags := ghoten.NewContext(coreOpts)
...
func (b *Local) localRunForPlanFile(ctx context.Context, op *backend.Operation, pf *planfile.Reader, run *backend.LocalRun, coreOpts *ghoten.ContextOpts, currentStateMeta *statemgr.SnapshotMeta) (*backend.LocalRun, *configload.Snapshot, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
...
// not something we expect folks to be relying on for routine use. Ghoten
...
// means.
func (b *Local) interactiveCollectVariables(ctx context.Context, existing map[string]backend.UnparsedVariableValue, vcs map[string]*configs.Variable, uiInput ghoten.UIInput) map[string]backend.UnparsedVariableValue {
	var needed []string
	...
	for _, name := range needed {
		...
		varUiInput := uiInput
		if vc.Ephemeral {
			varUiInput = ghoten.NewEphemeralSuffixUIInput(varUiInput)
		}
		rawValue, err := varUiInput.Input(ctx, &ghoten.InputOpts{
...
// to produce a complete Ghoten context for some ancillary functionality
...
func (v unparsedInteractiveVariableValue) ParseVariableValue(mode configs.VariableParsingMode) (*ghoten.InputValue, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
	...
	return &ghoten.InputValue{
		Value:      val,
		SourceType: ghoten.ValueFromInput,
	}, diags
}
...
func (v unparsedUnknownVariableValue) ParseVariableValue(mode configs.VariableParsingMode) (*ghoten.InputValue, tfdiags.Diagnostics) {
	return &ghoten.InputValue{
