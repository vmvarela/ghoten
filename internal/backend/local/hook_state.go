...
type IntermediateStatePersistInfo struct {
	...
	// persisted, or the time of the first report for Ghoten Core if there
	// hasn't yet been a persisted snapshot.
	LastPersist time.Time

	// ForcePersist is true when Ghoten CLI has received an interrupt
...
var _ ghoten.Hook = (*StateHook)(nil)

func (h *StateHook) PostStateUpdate(mutate func(*states.SyncState)) (ghoten.HookAction, error) {
	h.Lock()
	defer h.Unlock()
	...
	if h.StateMgr != nil {
		...
		if err != nil {
			return ghoten.HookActionHalt, err
		}
...
func (h *StateHook) Stopping() {
	...
	// If Ghoten has been asked to stop then that might mean that a hard
	// kill signal will follow shortly in case Ghoten doesn't stop
	...
	// do if they _do_ subsequently hard-kill Ghoten during an apply.
	...
	if mgrPersist, ok := h.StateMgr.(statemgr.Persister); ok && h.Schemas != nil {
		...
		// Ghoten Core won't start any new operations after it's been
		...
		if h.shouldPersist() {
			err := mgrPersist.PersistState(context.TODO(), h.Schemas)
			if err != nil {
				// This hook can't affect Ghoten Core's ongoing behavior,
...
// whether to create intermediate state snapshots when Ghoten Core emits
...
type IntermediateStateConditionalPersister interface {
	// ShouldPersistIntermediateState will be called each time Ghoten Core
