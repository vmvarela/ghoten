...
func TestLocal_plan_context_error(t *testing.T) {
	...
	// This is an intentionally-invalid value to make ghoten.NewContext fail
	// when b.Operation calls it.
	// NOTE: This test was originally using a provider initialization failure
	// as its forced error condition, but ghoten.NewContext is no longer
	// responsible for checking that. Invalid parallelism is the last situation
	// where ghoten.NewContext can return error diagnostics, and arguably
	// we should be validating this argument at the UI layer anyway, so perhaps
	// in future we'll make ghoten.NewContext never return errors and then
	// this test will become redundant, because its purpose is specifically
	// to test that we properly unlock the state if ghoten.NewContext
	// returns an error.
	if b.ContextOpts == nil {
		b.ContextOpts = &ghoten.ContextOpts{}
	}
...
You can apply this plan to save these new output values to the Ghoten state,
...
func TestLocal_planTainted(t *testing.T) {
	...
	expectedOutput := `Ghoten used the selected providers to generate the following execution plan.
...
func TestLocal_planDeposedOnly(t *testing.T) {
	...
	// is not common: it should arise only if Ghoten fails during
	...
	// as "current".
	//
	// However, that situation was more common in some earlier Ghoten
...
func TestLocal_planDestroy_withDataSources(t *testing.T) {
	...
	expectedOutput := `Ghoten will perform the following actions:
