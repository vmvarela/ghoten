...
// locally. This is the "default" backend and implements normal Ghoten
...
type Local struct {
	...
	// Ghoten context. Many of these will be overridden or merged by
...
//
// This will initialize an in-memory ghoten.Context to perform the
	...
	default:
		return nil, fmt.Errorf(
			"unsupported operation type: %s\n\n"+
				"This is a bug in Ghoten and should be reported. The local backend\n"+
				"is built-in to Ghoten and should always support all operations.",
			op.Type)
	}
...
Ghoten encountered an error attempting to save the state before cancelling the current operation. Once the operation is complete another attempt will be made to save the final state.`
