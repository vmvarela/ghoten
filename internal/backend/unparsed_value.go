...
type UnparsedVariableValue interface {
	...
	// or incomplete.
	ParseVariableValue(mode configs.VariableParsingMode) (*ghoten.InputValue, tfdiags.Diagnostics)
}
...
func ParseUndeclaredVariableValues(vv map[string]UnparsedVariableValue, decls map[string]*configs.Variable) (ghoten.InputValues, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
	ret := make(ghoten.InputValues, len(vv))
	...
	for name, rv := range vv {
		...
		case ghoten.ValueFromConfig, ghoten.ValueFromAutoFile, ghoten.ValueFromNamedFile:
...
func ParseDeclaredVariableValues(vv map[string]UnparsedVariableValue, decls map[string]*configs.Variable) (ghoten.InputValues, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
	ret := make(ghoten.InputValues, len(vv))
...
// Checks all given ghoten.InputValues variable maps for the existence of
// a named variable
func isDefinedAny(name string, maps ...ghoten.InputValues) bool {
	for _, m := range maps {
...
// to ghoten.NewContext. If the diagnostics contains errors, the returned
...
// partial result.
func ParseVariableValues(vv map[string]UnparsedVariableValue, decls map[string]*configs.Variable) (ghoten.InputValues, tfdiags.Diagnostics) {
	...
	// any we haven't gathered as unset placeholders which Ghoten Core
	// can then react to.
	for name, vc := range decls {
		...
		// This check is redundant with a check made in Ghoten Core when
		...
		// line options, whereas the one in Ghoten Core is more general
		...
		if vc.Required() {
			...
			// in the mapping to be explicit to Ghoten Core that we
			// visited it, but its value will be cty.NilVal to represent
			// that it wasn't set at all at this layer, and so Ghoten Core
