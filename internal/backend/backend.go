...
// Ghoten. A backend provides the abstraction that allows the same CLI
// to simultaneously support both local and remote operations for seamlessly
// using Ghoten in a team environment.
package backend
...
// Backend is the minimal interface that must be implemented to enable Ghoten.
...
type Enhanced interface {
	...
	// Operation performs a Ghoten operation such as refresh, plan, apply.
...
//
// This enables more behaviors of Ghoten that require more data such
...
// operations.
//
// The operation methods on ghoten.Context (Plan, Apply, Import, etc) each
...
// Ghoten operations have the same inputs and outputs even though they
...
type LocalRun struct {
	// Core is an already-initialized Ghoten Core context, ready to be
	...
	ApplyOpts *ghoten.ApplyOpts
}

// An operation represents an operation for Ghoten to execute.
//
...
// between Ghoten and Terraform Servers (a commercial product offered by
...
// against this interface and have Ghoten interact with it just as it
// would with HashiCorp-provided Terraform Servers.
type Operation struct {
	// Type is the operation to perform.
	Type OperationType

	// Encryption is used by enhanced backends for planning and ghoten.Context initialization
...
// that refers to a directory containing at least one Ghoten configuration
// file.
