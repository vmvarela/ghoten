...
// UIInput is an implementation of ghoten.UIInput that asks the CLI
// for input stdin.
type UIInput struct {
...
func (i *UIInput) Input(ctx context.Context, opts *ghoten.InputOpts) (string, error) {
	i.once.Do(i.init)
	...
	// Make sure we only ask for input once at a time. Ghoten
