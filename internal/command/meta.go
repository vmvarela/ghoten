...
import (
	...
	"github.com/vmvarela/ghoten/internal/command/webbrowser"
	"github.com/vmvarela/ghoten/internal/command/workdir"
	...
	"github.com/vmvarela/ghoten/internal/plugins"
	"github.com/vmvarela/ghoten/internal/providers"
	"github.com/vmvarela/ghoten/internal/provisioners"
...
type Meta struct {
	...
	// working directory of the Ghoten process, though this can take on
	...
	//
	// The intended use-case of this flag is when Ghoten is running in
	...
	// checksums inside to only be sufficient for the computer where Ghoten
	...
	// predating Ghoten, which Ghoten should use but not worry about the
	// lifecycle of.
	//
	// This is essentially a more extreme version of ProviderDevOverrides where
	// Ghoten doesn't even worry about how the provider server gets launched,
	// just trusting that someone else did it before running Ghoten.
	UnmanagedProviders map[addrs.Provider]*plugin.ReattachConfig

	// AllowExperimentalFeatures controls whether a command that embeds this
	// Meta is permitted to make use of experimental Ghoten features.
	//
	...
	// state even if the remote and local Ghoten versions don't match.
	...
	// Since `ghoten providers lock` and `tofu providers mirror` have their own
...
// ghoten.InputMode which is passed directly to Context.Input.
func (m *Meta) InputMode() ghoten.InputMode {
	if test || !m.input {
		return 0
	}
	...
	var mode ghoten.InputMode
	mode |= ghoten.InputModeProvider

	return mode
}

// UIInput returns a UIInput object to be used for asking for input.
func (m *Meta) UIInput() ghoten.UIInput {
	return &UIInput{
		Colorize: m.Colorize(),
	}
}
...
// mitchellh/cli rather than by Ghoten. Use this only in situations
...
// contextOpts returns the options to use to initialize a Ghoten
// context with the settings from this Meta.
func (m *Meta) contextOpts(ctx context.Context) (*ghoten.ContextOpts, error) {
	workspace, err := m.Workspace(ctx)
	if err != nil {
		return nil, err
	}

	var opts ghoten.ContextOpts
...
// the error when the configured Ghoten version on the remote workspace
// does not match the local Ghoten version.
func (m *Meta) ignoreRemoteVersionFlagSet(n string) *flag.FlagSet {
	f := m.defaultFlagSet(n)

	m.varFlagSet(f)

	f.BoolVar(&m.ignoreRemoteVersion, "ignore-remote-version", false, "continue even if remote and local Ghoten versions are incompatible")
...
func (m *Meta) confirm(opts *ghoten.InputOpts) (bool, error) {
	if !m.Input() {
...
func (m *Meta) showDiagnostics(vals ...interface{}) {
	...
	if m.compactWarnings {
		...
		if useCompact {
			...
			msg = "\n" + msg + "\nTo see the full warning notes, run Ghoten without -compact-warnings.\n"
			m.Ui.Warn(msg)
			return
		}
	}
...
// to set the name of the Ghoten workspace, overriding the workspace chosen
...
func (c *Meta) MaybeGetSchemas(ctx context.Context, state *states.State, config *configs.Config) (*ghoten.Schemas, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
	...
	if config != nil || state != nil {
		...
		tfCtx, ctxDiags := ghoten.NewContext(opts)
