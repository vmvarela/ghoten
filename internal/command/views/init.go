...
[reset][bold]Ghoten initialized in an empty directory![reset]
...
[reset]Ghoten encountered problems during initialization, including problems
...
func (v *InitHuman) InitSuccess(cloud bool) {
	if cloud {
		...
		const outputInitSuccess = `[reset][bold][green]Ghoten has been successfully initialized![reset][green]`
...
func (v *InitHuman) BuiltInProviderAvailable(provider string) {
	_, _ = v.view.streams.Println(fmt.Sprintf("- %s is built in to Ghoten", provider))
}
...
func (v *InitHuman) LockFileCreated() {
	_, _ = v.view.streams.Println(v.view.colorize.Color(`
Ghoten has created a lock file [bold].terraform.lock.hcl[reset] to record the provider
...
func (v *InitJSON) InitialisedFromEmptyDir() {
	const outputInitEmpty = `Ghoten initialized in an empty directory! The directory has no Ghoten configuration files. You may begin working with Ghoten immediately by creating Ghoten configuration files.`
	v.view.Info(outputInitEmpty)
}

func (v *InitJSON) ConfigError() {
	const errInitConfigError = `Ghoten encountered problems during initialization, including problems with the configuration, described below. The Ghoten configuration must be valid before initialization so that Ghoten can determine which modules and providers need to be installed.`
...
func (v *InitJSON) InitSuccess(cloud bool) {
	if cloud {
		...
		v.view.Info(`Ghoten has been successfully initialized!`)
	}
}

func (v *InitJSON) InitSuccessCLI(cloud bool) {
	if cloud {
		const outputInitSuccessCLICloud = `You may now begin working with cloud backend. Try running "ghoten plan" to see any changes that are required for your infrastructure. If you ever set or change modules or Ghoten Settings, run "ghoten init" again to reinitialize your working directory.`
		v.view.Info(outputInitSuccessCLICloud)
	} else {
		const outputInitSuccessCLI = `You may now begin working with Ghoten. Try running "ghoten plan" to see any changes that are required for your infrastructure. All Ghoten commands should now work. If you ever set or change modules or backend configuration for Ghoten, rerun this command to reinitialize your working directory. If you forget, other commands will detect it and remind you to do so if necessary.`
...
func (v *InitJSON) BuiltInProviderAvailable(provider string) {
	v.view.Info(fmt.Sprintf("%s is built in to Ghoten", provider))
}
...
func (v *InitJSON) LockFileCreated() {
	v.view.Info("Ghoten has created a lock file .terraform.lock.hcl to record the provider " +
		"selections it made above. Include this file in your version control repository " +
		"so that Ghoten can guarantee to make the same selections by default when " +
		"you run \"ghoten init\" in the future.")
}
...
func (v *InitJSON) LockFileChanged() {
	v.view.Info("Ghoten has made some changes to the provider dependency selections recorded " +
		"in the .terraform.lock.hcl file. Review those changes and commit them to your " +
		"version control system if they represent changes you intended to make.")
}
