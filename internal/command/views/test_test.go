...
Ghoten used the selected providers to generate the following execution plan.
...
some thing not very bad happened again
`,
			stderr: `Ghoten encountered an error destroying resources created while executing
main.tftest.hcl.
...
`,
			stderr: `
Ghoten left the following resources in state after executing main.tftest.hcl,
these left-over resources can be viewed by reading the statefile written to
disk(errored_test.tfstate) and they need to be cleaned up manually:
  - test.bar
  - test.bar (0fcb640a)
  - test.foo
`,
...
func TestTestHuman_FatalInterruptSummary(t *testing.T) {
	tcs := map[string]struct {
		...
		"no_state_only_plan": {
			...
			want: `
Ghoten was interrupted while executing main.tftest.hcl, and may not have
performed the expected cleanup operations.

Ghoten was in the process of creating the following resources for "run_block"
...
Ghoten has already created the following resources from the module under
test:
  - test_instance.one
  - test_instance.two
`,
...
Ghoten has already created the following resources for "setup_block" from
"../setup":
  - test_instance.one
  - test_instance.two
`,
	...
	tcs := map[string]struct {
		...
		"state_from_run": {
			...
			want: []map[string]interface{}{
				{
					"@level":    "error",
					"@message":  "Ghoten left some resources in state after executing main.tftest.hcl/run_block, these left-over resources can be viewed by reading the statefile written to disk(errored_test.tfstate) and they need to be cleaned up manually:",
					"@module":   "ghoten.ui",
					"@testfile": "main.tftest.hcl",
					"@testrun":  "run_block",
		...
		"state_only_warnings": {
			...
			want: []map[string]interface{}{
				{
					"@level":    "error",
					"@message":  "Ghoten left some resources in state after executing main.tftest.hcl, these left-over resources can be viewed by reading the statefile written to disk(errored_test.tfstate) and they need to be cleaned up manually:",
					"@module":   "ghoten.ui",
					"@testfile": "main.tftest.hcl",
					"test_cleanup": map[string]interface{}{
						"failed_resources": []interface{}{
	...
	tcs := map[string]struct {
		...
		"no_state_only_plan": {
			...
			want: []map[string]interface{}{
				{
					"@level":    "error",
					"@message":  "Ghoten was interrupted during test execution, and may not have performed the expected cleanup operations.",
					"@module":   "ghoten.ui",
					"@testfile": "main.tftest.hcl",
