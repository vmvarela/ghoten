...
func TestOperation_planNoChanges(t *testing.T) {
	...
	tests := map[string]struct {
		plan     func(schemas *ghoten.Schemas) *plans.Plan
		wantText string
	}{
		"nothing at all in normal mode": {
			func(schemas *ghoten.Schemas) *plans.Plan {
				return &plans.Plan{
		...
		"nothing at all in refresh-only mode": {
			...
			"Ghoten has checked that the real remote objects still match",
		},
		...
		"drift detected in normal mode": {
			...
			"Objects have changed outside of Ghoten",
		},
...
func TestOperation_planNextStep(t *testing.T) {
	testCases := map[string]struct {
		...
		"state path": {
			...
			want: `ghoten apply "good plan.tfplan"`,
		},
	}
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
