...
type View struct {
	...
	// When this is true it's a hint that Ghoten is being run indirectly
...
// suggest specific Ghoten commands to run, to make more conceptual gestures
// instead for situations where the user isn't running Ghoten directly.
//
...
func (v *View) Diagnostics(diags tfdiags.Diagnostics) {
	...
	// the filtering level is not ghoten.DeprecationWarningLevelAll.
	...
	if v.ModuleDeprecationWarnLvl != ghoten.DeprecationWarningLevelAll {
		var newDiags tfdiags.Diagnostics
		for _, diag := range diags {
			if !ghoten.DeprecationDiagnosticAllowed(v.ModuleDeprecationWarnLvl, diag) {
				continue
			}
