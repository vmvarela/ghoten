...
func (v *LoginHuman) TokenObtainedConfirmation(dispHostname string) {
	const msg = `[green][bold]Success![reset] [bold]Ghoten has obtained and saved an API token.[reset]
...
func (v *LoginHuman) OpeningBrowserForOAuth(dispHostname, authCodeURL string) {
	msg := fmt.Sprintf("Ghoten must now open a web browser to the login page for %s.\n", dispHostname)
	_, _ = v.view.streams.Println(msg)
...
func (v *LoginHuman) WaitingForHostSignal() {
	msg := "Ghoten will now wait for the host to signal that login was successful.\n"
	_, _ = v.view.streams.Println(msg)
}

func (v *LoginHuman) PasswordRequestHeader() {
	msg := "Ghoten must temporarily use your password to request an API token.\nThis password will NOT be saved locally.\n"
	_, _ = v.view.streams.Println(msg)
}

func (v *LoginHuman) OpeningBrowserForTokens(dispHostname, tokensURL string) {
	msg := fmt.Sprintf("Ghoten must now open a web browser to the tokens page for %s.\n", dispHostname)
	_, _ = v.view.streams.Println(msg)
	msg2 := fmt.Sprintf("If a browser does not open this automatically, open the following URL to proceed:\n    %s\n", tokensURL)
	_, _ = v.view.streams.Println(msg2)
}
...
func (v *LoginHuman) TokenStorageInHelper(helperType string) {
	msg := fmt.Sprintf("Ghoten will store the token in the configured %q credentials helper\nfor use by subsequent commands.\n", helperType)
	_, _ = v.view.streams.Println(msg)
}

func (v *LoginHuman) TokenStorageInFile(localFilename string) {
	msg := fmt.Sprintf("Ghoten will store the token in plain text in the following file\nfor use by subsequent commands:\n    %s\n", localFilename)
	_, _ = v.view.streams.Println(msg)
}
...
func (v *LoginHuman) RequestAPITokenMessage(dispHostname, mechanism string) {
	msg := fmt.Sprintf("Ghoten will request an API token for %s using %s.\n", dispHostname, mechanism)
	_, _ = v.view.streams.Println(msg)
}
...
func (v *LoginHuman) StorageLocationConsentInHelper(helperType string) {
	msg := fmt.Sprintf("If login is successful, Ghoten will store the token in the configured\n%q credentials helper for use by subsequent commands.\n", helperType)
	_, _ = v.view.streams.Println(msg)
}

func (v *LoginHuman) StorageLocationConsentInFile(localFilename string) {
	msg := fmt.Sprintf("If login is successful, Ghoten will store the token in plain text in\nthe following file for use by subsequent commands:\n    %s\n", localFilename)
	_, _ = v.view.streams.Println(msg)
}
...
func (v *LoginJSON) TokenObtainedConfirmation(dispHostname string) {
	const msg = `Success! Ghoten has obtained and saved an API token. The new API token will be used for any future Ghoten command that must make authenticated requests to %s`
	v.view.Info(fmt.Sprintf(msg, dispHostname))
}
...
func (v *LoginJSON) PasswordRequestHeader() {
	v.view.Info("Ghoten must temporarily use your password to request an API token. This password will NOT be saved locally.")
}

func (v *LoginJSON) OpeningBrowserForTokens(dispHostname, tokensURL string) {
	msg := fmt.Sprintf("Ghoten must now open a web browser to the login page for %[1]s: %[2]s. If a browser does not open this automatically, open the following URL to proceed: %[2]s", dispHostname, tokensURL)
	v.view.Info(msg)
}
...
func (v *LoginJSON) TokenStorageInHelper(helperType string) {
	v.view.Info(fmt.Sprintf("Ghoten will store the token in the configured %q credentials helper for use by subsequent commands", helperType))
}

func (v *LoginJSON) TokenStorageInFile(localFilename string) {
	v.view.Info(fmt.Sprintf("Ghoten will store the token in plain text in the following file for use by subsequent commands: %s", localFilename))
}
...
func (v *LoginJSON) RequestAPITokenMessage(dispHostname, mechanism string) {
	v.view.Info(fmt.Sprintf("Ghoten will request an API token for %s using %s", dispHostname, mechanism))
}
...
func (v *LoginJSON) StorageLocationConsentInHelper(helperType string) {
	v.view.Info(fmt.Sprintf("If login is successful, Ghoten will store the token in the configured %q credentials helper for use by subsequent commands", helperType))
}

func (v *LoginJSON) StorageLocationConsentInFile(localFilename string) {
	v.view.Info(fmt.Sprintf("If login is successful, Ghoten will store the token in plain text in the following file for use by subsequent commands: %s", localFilename))
}
