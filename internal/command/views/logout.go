...
func (v *LogoutHuman) LogoutSuccess(dispHostname string) {
	msg := fmt.Sprintf(v.view.colorize.Color("[green][bold]Success![reset] [bold]Ghoten has removed the stored API token for %s.[reset]"), dispHostname)
	_, _ = v.view.streams.Println(msg + "\n")
}
...
func (v *LogoutJSON) LogoutSuccess(dispHostname string) {
	v.view.Info(fmt.Sprintf("Success! Ghoten has removed the stored API token for %s", dispHostname))
}
