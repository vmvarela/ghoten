...
type OperationHuman struct {
	...
	// run a follow-up command, perhaps because Ghoten is running in
...
const interrupted = `
Interrupt received.
Please wait for Ghoten to exit or data loss may occur.
Gracefully shutting down...
`

const planHeaderNoOutput = `
Note: You didn't use the -out option to save this plan, so Ghoten can't guarantee to take exactly these actions if you run "ghoten apply" now.
`
...
Ghoten has generated configuration and written it to %s. Please review the configuration and edit it as necessary before adding it to version control.
`
