...
import (
	...
	terraformProvider "github.com/vmvarela/ghoten/internal/builtin/providers/tf"
	"github.com/vmvarela/ghoten/internal/getproviders"
	"github.com/vmvarela/ghoten/internal/logging"
	tfplugin "github.com/vmvarela/ghoten/internal/plugin"
	tfplugin6 "github.com/vmvarela/ghoten/internal/plugin6"
...
// EnsureProviderVersions (anything other than "ghoten init") can safely
...
//
// This is used to implement the -plugin-dir option for "ghoten init", where
...
// locations for versions of Ghoten providers that are available for
...
// Users may choose to override this during a "ghoten init" command by
...
func (m *Meta) providerDevOverrideInitWarnings() tfdiags.Diagnostics {
	...
	detailMsg.WriteString("\nSkip ghoten init when using provider development overrides. It is not necessary and may error unexpectedly.")
...
func (m *Meta) providerFactories() (map[addrs.Provider]providers.Factory, error) {
	...
	// available in the provider cache because "ghoten init" should already
	// have put them there.
	...
	// call "unmanaged" because Ghoten isn't responsible for starting
	...
	// and orchestrating one or more non-interactive Ghoten runs that then
	// exercise it.
	...
	for provider, lock := range providerLocks {
		...
		checkProvider := func() error {
			...
			if allowedHashes := lock.PreferredHashes(); len(allowedHashes) != 0 {
				...
				if !matched {
					return fmt.Errorf(
						"the cached package for %s %s (in %s) does not match any of the checksums recorded in the dependency lock file, run ghoten init to ensure all providers are correctly installed",
						provider, version, cacheDir.BasePath(),
					)
				}
			}
