...
func (c *UnlockCommand) Run(args []string) int {
	...
	if !force {
		...
		desc := "Ghoten will remove the lock on the remote state.\n" +
			"This will allow local Ghoten commands to modify this state, even though it\n" +
			"may still be in use. Only 'yes' will be accepted to confirm."

		v, err := c.UIInput().Input(context.Background(), &ghoten.InputOpts{
			Id:          "force-unlock",
...
[reset][bold][green]Ghoten state has been successfully unlocked![reset][green]

The state has been unlocked, and Ghoten commands should now be able to
