...
type DiagnosticSnippet struct {
	...
	// Code is a possibly-multi-line string of Ghoten configuration, which
...
func newDiagnosticSnippet(snippetRange, highlightRange *tfdiags.SourceRange, sources map[string]*hcl.File) *DiagnosticSnippet {
	...
	if !ok {
		...
		// a source file, like an expression entered into the "ghoten console" prompt.)
		return nil
	}
	src := file.Bytes
	if src == nil {
...
// For context: https://github.com/vmvarela/ghoten/issues/2545
