...
import (
	...
	pluginDiscovery "github.com/vmvarela/ghoten/internal/plugin/discovery"
...
func collectCredentialsFromEnv() map[svchost.Hostname]string {
	...
	for _, ev := range os.Environ() {
		...
		// that is what the Ghoten credentials helper protocol will
		// use in its requests.
		//
		// Using ForDisplay first here makes this more liberal than Ghoten
		// itself would usually be in that it will tolerate pre-punycoded
		// hostnames that Ghoten normally rejects in other contexts in order
