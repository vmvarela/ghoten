	...
	tests := []locationTest{
		{
			locationTestParameters: locationTestParameters{
				name:  ".ghotenrc only",
				files: []string{filepath.Join(home, ".ghotenrc")},
			},
		...
		{
			locationTestParameters: locationTestParameters{
				name:  ".terraformrc and .ghotenrc",
				files: []string{filepath.Join(home, ".terraformrc"), filepath.Join(home, ".ghotenrc")},
			},
			expected: map[string]*ConfigHost{
				"config1.example.com": {
					Services: map[string]interface{}{
		...
		{
			locationTestParameters: locationTestParameters{
				name:        "xdg directory, but with .ghotenrc and .terraformrc present",
				files:       []string{filepath.Join(home, ".terraformrc"), filepath.Join(home, ".ghotenrc"), filepath.Join(xdgDir, "ghoten", "ghotenrc")},
		...
		{
			locationTestParameters: locationTestParameters{
				name:        "xdg directory without .ghotenrc and .terraformrc present",
				files:       []string{filepath.Join(xdgDir, "ghoten", "ghotenrc")},
				directories: []string{xdgDir},
				envVars:     map[string]string{"XDG_CONFIG_HOME": xdgDir},
			},
			expected: map[string]*ConfigHost{
		...
		},
		{
			locationTestParameters: locationTestParameters{
				...
				files:   []string{filepath.Join(home, "mytofufile"), filepath.Join(home, ".terraformrc"), filepath.Join(home, ".ghotenrc")},
...
func TestConfigDirLocations(t *testing.T) {
	...
	tests := []directoryLocationTest{
		...
		},
		{
			...
			expected: []string{filepath.Join(xdgDir, "ghoten")},
		},
		{
			locationTestParameters: locationTestParameters{
...
func TestDataDirLocations(t *testing.T) {
	...
	tests := []directoryLocationTest{
		...
		},
		{
			...
			expected: []string{terraformD, filepath.Join(xdgDir, "ghoten")},
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
