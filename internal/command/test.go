...
Usage: ghoten [global options] test [options]
  ...
  Executes automated integration tests against the current Ghoten 
  configuration.

  Ghoten will search for .tftest.hcl files within the current configuration 
  and testing directories. Ghoten will then execute the testing run blocks 
...
Options:
  ...
  -filter=testfile      If specified, Ghoten will only execute the test files
...
type TestSuiteRunner struct {
	...
	// StoppedCtx and CancelledCtx allow in progress Ghoten operations to
		...
		if run.Config.ConfigUnderTest != nil {
			...
			if key == MainStateIdentifier {
				...
				run.Diagnostics = run.Diagnostics.Append(&hcl.Diagnostic{
					...
					Detail:   fmt.Sprintf("The source for the selected module evaluated to %s which should not be possible. This is a bug in Ghoten - please report it!", key),
		...
		if runner.Suite.Verbose {
			...
			if diags.HasErrors() {
				...
				diags = diags.Append(tfdiags.Sourceless(
					...
					fmt.Sprintf("Ghoten failed to print the verbose output for %s, other diagnostics will contain more details as to why.", path.Join(file.Name, run.Name))))
			} else {
...
func (runner *TestFileRunner) validate(ctx context.Context, config *configs.Config, run *moduletest.Run, file *moduletest.File) tfdiags.Diagnostics {
	...
	tfCtx, ctxDiags := ghoten.NewContext(runner.Suite.Opts)
...
func (runner *TestFileRunner) plan(ctx context.Context, config *configs.Config, state *states.State, run *moduletest.Run, file *moduletest.File) (*ghoten.Context, *plans.Plan, tfdiags.Diagnostics) {
...
func (runner *TestFileRunner) apply(ctx context.Context, plan *plans.Plan, state *states.State, config *configs.Config, run *moduletest.Run, file *moduletest.File) (*ghoten.Context, *states.State, tfdiags.Diagnostics) {
...
func (runner *TestFileRunner) wait(ctx *ghoten.Context, runningCtx context.Context, run *moduletest.Run, file *moduletest.File, created []*plans.ResourceInstanceChangeSrc) (diags tfdiags.Diagnostics, cancelled bool) {
...
func (runner *TestFileRunner) Cleanup(ctx context.Context, file *moduletest.File) {
	...
	for key, state := range runner.States {
		if state.Run == nil {
			...
			var diags tfdiags.Diagnostics
			diags = diags.Append(tfdiags.Sourceless(tfdiags.Error, "Inconsistent state", fmt.Sprintf("Found inconsistent state while cleaning up %s. This is a bug in Ghoten - please report it", file.Name)))
...
// TestFileState instances, configuration and global variables. Also, creates a ghoten.InputValues mapping for
...
// buildInputVariablesForTest creates a ghoten.InputValues mapping for
...
func buildInputVariablesForTest(run *moduletest.Run, file *moduletest.File, config *configs.Config, globals map[string]backend.UnparsedVariableValue, evalCtx *hcl.EvalContext) (ghoten.InputValues, tfdiags.Diagnostics) {
		...
		if run != nil {
			if expr, exists := run.Config.Variables[name]; exists {
				...
				variables[name] = testVariableValueExpression{
					expr:       expr,
					sourceType: ghoten.ValueFromConfig,
					ctx:        evalCtx,
				}
				continue
			}
...
type testVariableValueExpression struct {
	expr       hcl.Expression
	sourceType ghoten.ValueSourceType
	ctx        *hcl.EvalContext
}

func (v testVariableValueExpression) ParseVariableValue(mode configs.VariableParsingMode) (*ghoten.InputValue, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
...
// parseAndApplyDefaultValues parses the given unparsed variables into ghoten.InputValues
// and applies default values from the configuration variables where applicable.
// This ensures all variables are correctly initialized and returns the resulting ghoten.InputValues.
func parseAndApplyDefaultValues(unparsedVariables map[string]backend.UnparsedVariableValue, configVariables map[string]*configs.Variable) (ghoten.InputValues, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
	inputs := make(ghoten.InputValues, len(unparsedVariables))
	...
	// We do this after the conversion into ghoten.InputValues, as the
...
// prepareInputVariablesForAssertions creates a ghoten.InputValues mapping
...
func (runner *TestFileRunner) prepareInputVariablesForAssertions(config *configs.Config, run *moduletest.Run, file *moduletest.File, globals map[string]backend.UnparsedVariableValue) (ghoten.InputValues, func(), tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
	...
	// ghoten.InputValues so they can be passed into the Ghoten graph.
