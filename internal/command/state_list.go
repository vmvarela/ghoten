...
Usage: ghoten [global options] state (list|ls) [options] [address...]
  ...
  List resources in the Ghoten state.

  This command lists resource instances in the Ghoten state. The address
...
Options:
  ...
  -state=statefile    Path to a Ghoten state file to use to look
                      up Ghoten-managed resources. By default, Ghoten
...
Please ensure that your Ghoten state exists and that you've
configured it properly. You can use the "-state" flag to point
Ghoten at another state file.`

const errStateNotFound = `No state file was found!

State management commands require a state file. Run this command
in a directory where Ghoten has been run or use the -state flag
