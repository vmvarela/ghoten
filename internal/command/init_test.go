...
func TestInit_backendCli_no_config_block(t *testing.T) {
	...
	// The was not correct in terms of Ghoten configuration. The Ui configured for Ghoten, sends the warning diagnostics
	// to stdout and not stderr. (see https://github.com/vmvarela/ghoten/blob/db8c872defd8666618649ef7e29fa2b809adfd5e/cmd/tofu/main.go#L49-L51)
	...
	// Before the change, the MockUi used in this test was not considering this particularity of Ghoten.
...
func TestInit_backendCloudInvalidOptions(t *testing.T) {
	...
	t.Run("-backend-config", func(t *testing.T) {
		...
		// where Ghoten is running, such as AWS authentication profiles
		// that are naturally local only to the machine where Ghoten is
...
func TestInit_getProviderDetectedDuplicate(t *testing.T) {
	...
	warnings := []string{
		...
		"Ghoten has detected multiple providers of type foo",
		"If this is intentional you can ignore this warning",
	}
	unexpected := []string{
		"Ghoten has detected multiple providers of type bar",
	}
...
func TestInit_providerSource(t *testing.T) {
	...
	if strings.Contains(output.Stdout(), "Ghoten has initialized, but configuration upgrades may be needed") {
	...
	// The previous used output here was the stderr. That was correct in terms of the MockUi (and cli.BasicUi) implementation.
	// The was not correct in terms of Ghoten configuration. The Ui configured for Ghoten, sends the warning diagnostics
	// to stdout and not stderr. (seehttps://github.com/vmvarela/ghoten/blob/db8c872defd8666618649ef7e29fa2b809adfd5e/cmd/tofu/main.go#L49-L51)
	// This change from stderr to stdout has been done while we migrated the init command to use views, which the underlying
	// logic, writes the warnings to stdout.
	// Before the change, the MockUi used in this test was not considering this particularity of Ghoten.
...
func TestInit_checkRequiredVersionFirst(t *testing.T) {
	t.Run("root_module", func(t *testing.T) {
		...
		errStr := output.Stderr()
		if !strings.Contains(errStr, `Unsupported Ghoten Core version`) {
			t.Fatalf("output should point to unmet version constraint, but is:\n\n%s", errStr)
		}
	})
...
func TestInit_pluginDirWithBuiltIn(t *testing.T) {
	...
	if subStr := "terraform.io/builtin/terraform is built in to Ghoten"; !strings.Contains(outputStr, subStr) {
...
func TestInit_invalidBuiltInProviders(t *testing.T) {
	...
	if subStr := "Cannot use terraform.io/builtin/nonexist: this Ghoten release"; !strings.Contains(errStr, subStr) {
...
func TestInit_invalidSyntaxNoBackend(t *testing.T) {
	...
	errStr := output.Stderr()
	if subStr := "Ghoten encountered problems during initialization, including problems\nwith the configuration, described below."; !strings.Contains(errStr, subStr) {
