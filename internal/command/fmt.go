...
// FmtCommand is a Command implementation that rewrites Ghoten config
...
func (c *FmtCommand) fmt(paths []string, stdin io.Reader, stdout io.Writer) tfdiags.Diagnostics {
	...
	for _, path := range paths {
		...
		if info.IsDir() {
			...
			if !fmtd {
				diags = diags.Append(fmt.Errorf("Only .tf, .tfvars, and .tftest.hcl files can be processed with ghoten fmt"))
				continue
			}
		}
	}
...
func (c *FmtCommand) processFile(path string, r io.Reader, w io.Writer, isStdout bool) tfdiags.Diagnostics {
	...
	log.Printf("[TRACE] ghoten fmt: Formatting %s", path)
...
func (c *FmtCommand) processDir(path string, stdout io.Writer) tfdiags.Diagnostics {
	...
	log.Printf("[TRACE] ghoten fmt: looking for files in %s", path)
	...
	for _, info := range entries {
		...
		if info.IsDir() {
			...
			// want to mimic the file-reading behavior of "ghoten plan", etc,
...
Usage: ghoten [global options] fmt [options] [target...]
  ...
  Rewrites all Ghoten configuration files to a canonical format. All
  ...
  The content must be in the Ghoten language native syntax; JSON is not
  supported.
