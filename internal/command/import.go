...
func (c *ImportCommand) Run(args []string) int {
	...
	cmdFlags.BoolVar(&c.ignoreRemoteVersion, "ignore-remote-version", false, "continue even if remote and local Ghoten versions are incompatible")
	...
	if !c.dirIsConfigPath(configPath) {
		diags = diags.Append(&hcl.Diagnostic{
			...
			Summary:  "No Ghoten configuration files",
			Detail: fmt.Sprintf(
				"The directory %s does not contain any Ghoten configuration files (.tf or .tf.json). To specify a different configuration directory, use the -config=\"...\" command line option.",
				configPath,
			),
		})
		c.showDiagnostics(diags)
	...
	// import something that Ghoten will want to immediately destroy on
	...
	opReq.Hooks = []ghoten.Hook{c.uiHook()}
	{
	...
	// Check remote Ghoten version is compatible
	...
	newState, importDiags := lr.Core.Import(ctx, lr.Config, lr.InputState, &ghoten.ImportOpts{
		Targets: []*ghoten.ImportTarget{
			{
				CommandLineImportTarget: &ghoten.CommandLineImportTarget{
					Addr: addr,
					ID:   args[1],
				},
			},
...
Usage: ghoten [global options] import [options] ADDR ID
  ...
  Import existing infrastructure into your Ghoten state.

  This will find and import the specified resource into your Ghoten
  state, allowing existing infrastructure to come under Ghoten
  management without having to be initially created by Ghoten.
...
Options:
  ...
  -config=path            Path to a directory of Ghoten configuration files
...
your Ghoten state and will henceforth be managed by Ghoten.
`
