...
// Dir represents a single Ghoten working directory.
//
...
type Dir struct {
	...
	// current working directory when running Ghoten CLI, or the
	// directory explicitly chosen by the user using the -chdir=...
	// global option.
	mainDir string

	// originalDir is the path to the working directory that was
	// selected when creating the Ghoten CLI process, regardless of
...
//
// Use this only to record the original working directory when Ghoten is run
...
// directory that the current Ghoten process was launched from.
//
// This is usually the same as the main working directory, but differs in the
// special case where the user ran Ghoten with the global -chdir=...
...
// running Ghoten, readable and writable by that user, and readable by
