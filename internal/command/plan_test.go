...
//     changes outside of Ghoten (if any) might be interesting to call out
//     in the plan diff UI.
...
//     Ghoten" only to changes that seem like they might have contributed to
...
func TestPlan_withInvalidReferencesInTry(t *testing.T) {
	...
	provider := &ghoten.MockProvider{
		...
		ReadResourceFn: func(req providers.ReadResourceRequest) providers.ReadResourceResponse {
			...
			// language runtime would recognize as "changes outside of Ghoten",
...
// step just passing through the new object proposed by Ghoten Core.
func planFixtureProvider() *ghoten.MockProvider {
	p := testProvider()
...
// GetSchemaResponse and PlanResourceChangeFn populated, with the plan
// step just passing through the new object proposed by Ghoten Core.
func planVarsFixtureProvider() *ghoten.MockProvider {
	p := testProvider()
...
func planWarningsFixtureProvider() *ghoten.MockProvider {
	p := testProvider()
	p.GetProviderSchemaResponse = planFixtureSchema()
	p.PlanResourceChangeFn = func(req providers.PlanResourceChangeRequest) providers.PlanResourceChangeResponse {
		return providers.PlanResourceChangeResponse{
