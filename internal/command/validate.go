...
func (c *ValidateCommand) Run(rawArgs []string) int {
	...
	// the user is trying to use "ghoten validate" as a sort of pre-flight
...
func (c *ValidateCommand) validate(ctx context.Context, dir, testDir string, noTests bool) tfdiags.Diagnostics {
	...
	// We'll also do a quick validation of the Ghoten test files. These live
	// outside the Ghoten graph so we have to do this separately.
...
Options:
  ...
  -no-tests             If specified, Ghoten will not validate test files.
