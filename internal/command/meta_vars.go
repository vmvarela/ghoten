	...
	// precedence.
	{
		...
		for _, raw := range env {
			...
			ret[name] = unparsedVariableValueString{
				str:        rawVal,
				name:       name,
				sourceType: ghoten.ValueFromEnvVar,
			}
		}
	}
	...
	for _, rawFlag := range m.variableArgs.AllItems() {
		...
		case "-var":
			...
			ret[name] = unparsedVariableValueString{
				str:        rawVal,
				name:       name,
				sourceType: ghoten.ValueFromCLIArg,
			}

		case "-var-file":
			moreDiags := m.addVarsFromFile(rawFlag.Value, ghoten.ValueFromNamedFile, ret)
			diags = diags.Append(moreDiags)

		default:
			// Should never happen; always a bug in the code that built up
			// the contents of m.variableArgs.
			diags = diags.Append(fmt.Errorf("unsupported variable option name %q (this is a bug in Ghoten)", rawFlag.Name))
		}
	}
...
func (m *Meta) addVarsFromDir(currDir string, ret map[string]backend.UnparsedVariableValue) tfdiags.Diagnostics {
	...
	if _, err := os.Stat(filepath.Join(currDir, DefaultVarsFilename)); err == nil {
		moreDiags := m.addVarsFromFile(filepath.Join(currDir, DefaultVarsFilename), ghoten.ValueFromAutoFile, ret)
		diags = diags.Append(moreDiags)
	}
	...
	if _, err := os.Stat(filepath.Join(currDir, defaultVarsFilenameJSON)); err == nil {
		moreDiags := m.addVarsFromFile(filepath.Join(currDir, defaultVarsFilenameJSON), ghoten.ValueFromAutoFile, ret)
		diags = diags.Append(moreDiags)
	}
	if infos, err := os.ReadDir(currDir); err == nil {
		...
		for _, info := range infos {
			...
			moreDiags := m.addVarsFromFile(filepath.Join(currDir, name), ghoten.ValueFromAutoFile, ret)
			diags = diags.Append(moreDiags)
		}
	}
...
func (m *Meta) addVarsFromFile(filename string, sourceType ghoten.ValueSourceType, to map[string]backend.UnparsedVariableValue) tfdiags.Diagnostics {
	var diags tfdiags.Diagnostics
...
func (v unparsedVariableValueExpression) ParseVariableValue(mode configs.VariableParsingMode) (*ghoten.InputValue, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
...
func (v unparsedVariableValueString) ParseVariableValue(mode configs.VariableParsingMode) (*ghoten.InputValue, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
