...
func (c *StateShowCommand) Run(args []string) int {
	...
	migratedState, migrateDiags := ghotenmigrate.MigrateStateProviderAddresses(lr.Config, state)
...
Usage: ghoten [global options] state show [options] ADDRESS
  ...
  Shows the attributes of a resource in the Ghoten state.

  This command shows the attributes of a single resource in the Ghoten
  state. The address argument must be used to specify a single resource.
  You can view the list of available resources with "ghoten state list".

Options:

  -state=statefile    Path to a Ghoten state file to use to look
                      up Ghoten-managed resources. By default it will
...
To view the available instances, use "ghoten state list". Please modify 
