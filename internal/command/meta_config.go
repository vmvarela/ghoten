...
func (m *Meta) getInput(ctx context.Context, variable *configs.Variable) (string, error) {
	...
	if variable.Ephemeral {
		uiInput = ghoten.NewEphemeralSuffixUIInput(uiInput)
	}
	rawValue, err := uiInput.Input(ctx, &ghoten.InputOpts{
...
// least one Ghoten configuration file (.tf or .tf.json), returning true
// if so.
//
...
func (m *Meta) inputForSchema(given cty.Value, schema *configschema.Block) (cty.Value, error) {
	...
	for _, name := range names {
		...
		for {
			strVal, err := input.Input(context.Background(), &ghoten.InputOpts{
				Id:          name,
				Query:       name,
...
// of most Ghoten features, so callers should report any error and safely
// terminate.
