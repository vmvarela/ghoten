...
func (c *WorkspaceCommand) Help() string {
	helpText := `
Usage: ghoten [global options] workspace

  new, list, show, select and delete Ghoten workspaces.
...
func envCommandShowWarning(ui cli.Ui, show bool) {
	...
	ui.Warn(`Warning: the "ghoten env" family of commands is deprecated.

"Workspace" is now the preferred term for what earlier Ghoten versions
called "environment", to reduce ambiguity caused by the latter term colliding
with other concepts.

The "ghoten workspace" commands should be used instead. "ghoten env"
will be removed in a future Ghoten version.
`)
}
...
so if you run "ghoten plan" Ghoten will not see any existing state
...
but are no longer manageable by Ghoten since the state has
been deleted.
`
