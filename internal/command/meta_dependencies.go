...
// corresponding provider in Ghoten's registry, to keep dependency selections
...
// installation based on the result, such as the implementation "ghoten init".
...
func (m *Meta) lockedDependenciesWithPredecessorRegistryShimmed() (*depsfile.Locks, tfdiags.Diagnostics) {
	...
	// If this is the first run after switching from Ghoten's predecessor,
	// the lock file might contain some entries from the predecessor's registry
	// which we can translate into similar entries for Ghoten's registry.
	...
	if len(changed) != 0 {
		...
		_, _ = buf.WriteString("Ghoten automatically rewrote some entries in your dependency lock file:\n")
		...
		_, _ = buf.WriteString("\nThe version selections were preserved, but the hashes were not because the Ghoten project's provider releases are not byte-for-byte identical.")
