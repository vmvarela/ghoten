...
import (
	...
	"github.com/vmvarela/ghoten/internal/flock"
...
type LocalState struct {
	...
	mu        sync.Mutex
	state     *ghoten.State
	readState *ghoten.State
	written   bool
}

// SetState will force a specific state in-memory for this local state.
func (s *LocalState) SetState(state *ghoten.State) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
// StateReader impl.
func (s *LocalState) State() *ghoten.State {
...
//
// StateWriter impl.
func (s *LocalState) WriteState(state *ghoten.State) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *LocalState) writeState(state *ghoten.State) error {
	...
	if err := ghoten.WriteState(s.state, s.stateFileOut); err != nil {
		return err
	}
...
func (s *LocalState) RefreshState(_ context.Context) error {
	...
	state, err := ghoten.ReadState(reader)
	// if there's no state we just assign the nil return value
	if err != nil && err != ghoten.ErrNoState {
		return err
	}
