...
import (
	...
	backendLocal "github.com/vmvarela/ghoten/internal/backend/local"
	legacy "github.com/vmvarela/ghoten/internal/legacy/ghoten"
)
...
//
// The backend is used to perform the actual Ghoten operations. This
// abstraction enables easily sliding in new Ghoten behavior such as
...
// the "ghoten init" command line, etc.
func (m *Meta) Backend(ctx context.Context, opts *BackendOpts, enc encryption.StateEncryption) (backend.Enhanced, tfdiags.Diagnostics) {
	...
	if m.AllowExperimentalFeatures {
		...
		ghoten.SetExperimentalRuntimeAllowed(true)
	}
		...
		if errs := providerPluginErrors(nil); errors.As(err, &errs) {
			...
			if !opts.Init {
				...
				if m.RunningInAutomation {
					// Don't mention "ghoten init" specifically if we're running in an automation wrapper
					suggestion = "You must install the required plugins before running Ghoten operations."
				}
				diags = diags.Append(tfdiags.Sourceless(
					tfdiags.Error,
					"Required plugins are not installed",
					fmt.Sprintf(
						"The installed provider plugins are not consistent with the packages selected in the dependency lock file:%s\n\nGhoten uses external plugins to integrate with a variety of different infrastructure services. %s",
...
func (m *Meta) selectWorkspace(ctx context.Context, b backend.Backend) error {
	...
	if len(workspaces) == 0 {
		if c, ok := b.(*cloud.Cloud); ok && m.input {
			...
			// let's just ask:
			name, err := m.UIInput().Input(context.Background(), &ghoten.InputOpts{
				Id:          "create-workspace",
	...
	v, err := m.UIInput().Input(context.Background(), &ghoten.InputOpts{
		Id: "select-workspace",
		Query: fmt.Sprintf(
...
func (m *Meta) BackendForLocalPlan(ctx context.Context, settings plans.Backend, enc encryption.StateEncryption) (backend.Enhanced, tfdiags.Diagnostics) {
	...
	if canonType != settings.Type {
		...
		// this plan.
		diags = diags.Append(fmt.Errorf("saved plan should use canonical backend type %q, not alias %q; this is a bug in Ghoten", canonType, settings.Type))
		return nil, diags
	}
	b := f(enc)
...
func (m *Meta) backendFromConfig(ctx context.Context, opts *BackendOpts, enc encryption.StateEncryption) (backend.Backend, tfdiags.Diagnostics) {
	...
	// structures in the Ghoten package. It is not actually a Ghoten
	...
	// otherwise-obsolete ghoten.State.
	// ------------------------------------------------------------------------
	...
	if !s.Remote.Empty() {
		...
		diags = diags.Append(tfdiags.Sourceless(
			...
			"This working directory is configured for legacy remote state, which is no longer supported from Terraform v0.12 onwards, and thus not supported by Ghoten, either. To migrate this environment, first run \"terraform init\" under a Terraform 0.11 release, and then upgrade to Ghoten.",
		))
		return nil, diags
	}
	...
	case c == nil && !s.Backend.Empty():
		...
		if !opts.Init {
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				"Backend initialization required, please run \"ghoten init\"",
	...
	case c != nil && s.Backend.Empty():
		...
		if !opts.Init {
			if c.Type == "cloud" {
				...
				diags = diags.Append(tfdiags.Sourceless(
					tfdiags.Error,
					"Cloud backend initialization required: please run \"ghoten init\"",
...
func (m *Meta) backendFromState(ctx context.Context, enc encryption.StateEncryption) (backend.Backend, tfdiags.Diagnostics) {
	...
	if canonType != s.Backend.Type {
		...
		diags = diags.Append(fmt.Errorf("working directory is configured for backend alias %q instead of the canonical name %q; this is a bug in Ghoten", s.Backend.Type, canonType))
		return nil, diags
	}
	b := f(enc)
	...
	if err != nil {
		diags = diags.Append(tfdiags.Sourceless(
			...
			fmt.Sprintf("The backend configuration created by the most recent run of \"ghoten init\" could not be decoded: %s. The configuration may have been initialized by an earlier version that used an incompatible configuration structure. Run \"ghoten init -reconfigure\" to force re-initialization of the backend.", err),
		))
		return nil, diags
	}
...
func (m *Meta) savedBackend(ctx context.Context, sMgr *clistate.LocalState, enc encryption.StateEncryption) (backend.Backend, tfdiags.Diagnostics) {
	...
	if s.Backend.Type != canonName {
		...
		diags = diags.Append(fmt.Errorf("working directory state uses alias %q instead of canonical backend type %q; this is a bug in Ghoten", s.Backend.Type, canonName))
		return nil, diags
	}
	b := f(enc)
...
func (m *Meta) backendInitFromConfig(ctx context.Context, c *configs.Backend, enc encryption.StateEncryption) (backend.Backend, cty.Value, tfdiags.Diagnostics) {
	...
	if c.Type != canonType {
		diags = diags.Append(fmt.Errorf("backend configuration still contains alias type %q instead of canonical %q; this is a bug in Ghoten", c.Type, canonType))
		return nil, cty.NilVal, diags
	}
	b := f(enc)
...
// Helper method to check the local Ghoten version against the configured
...
func (m *Meta) assertSupportedCloudInitOptions(mode cloud.ConfigChangeMode) tfdiags.Diagnostics {
	...
	if mode.InvolvesCloud() {
		...
		if m.migrateState {
			name := "-migrate-state"
			if m.forceInitCopy {
				// -force copy implies -migrate-state in "ghoten init",
...
Ghoten is trying to read your local state to determine if there is
state to migrate to your newly configured backend. Ghoten can't continue
...
This is the backend specified in your Ghoten configuration file.
This error could be a simple typo in your configuration, but it can also
be caused by using a Ghoten version that doesn't support the specified
backend type. Please check your configuration and your Ghoten version.

If you'd like to run Ghoten and store state locally, you can fix this
...
Use the "ghoten workspace" command to create and select a new workspace.
...
This is the backend that this Ghoten environment is configured to use
both in your configuration and saved locally as your last-used backend.
If it isn't found, it could mean an alternate version of Ghoten was
used with this configuration. Please use the proper version of Ghoten that
contains support for this backend.

If you'd like to force remove this backend, you must update your configuration
to not use the backend and run "ghoten init" (or any other command) again.
`

const errBackendClearSaved = `
Error clearing the backend configuration: %w

Ghoten removes the saved backend configuration when you're removing a
configured backend. This must be done so future Ghoten runs know to not
...
The "backend" is the interface that Ghoten uses to store state,
perform operations, etc. If this message is showing up, it means that the
Ghoten configuration you're using is using a custom configuration for
the Ghoten backend.

Changes to backend configurations require reinitialization. This allows
Ghoten to set up the new configuration, copy existing state, etc. Please run
"ghoten init" with either the "-reconfigure" or "-migrate-state" flags to
...
Ghoten has not yet made changes to your existing configuration or state.
`

const errBackendWriteSaved = `
Error saving the backend configuration: %w

Ghoten saves the complete backend configuration in a local file for
...
Ghoten detected that the backend type changed from %q to %q.
`

const outputBackendMigrateLocal = `
Ghoten has detected you're unconfiguring your previously set %q backend.
`

const outputBackendReconfigure = `
[reset][bold]Backend configuration changed![reset]

Ghoten has detected that the configuration specified for the backend
has changed. Ghoten will now check for existing state in the backends.
`

const inputCloudInitCreateWorkspace = `
There are no workspaces with the configured tags (%s)
in your cloud backend organization. To finish initializing, Ghoten needs at
least one workspace available.

Ghoten can create a properly tagged workspace for you now. Please enter a
name to create a new cloud backend workspace.
`

const successBackendUnset = `
Successfully unset the backend %q. Ghoten will now operate locally.
`

const successBackendSet = `
Successfully configured the backend %q! Ghoten will automatically
...
var migrateOrReconfigDiag = tfdiags.Sourceless(
	...
	"A change in the backend configuration has been detected, which may require migrating existing state.\n\n"+
		"If you wish to attempt automatic migration of the state, use \"ghoten init -migrate-state\".\n"+
		`If you wish to store the current configuration with no changes to the state, use "ghoten init -reconfigure".`)
