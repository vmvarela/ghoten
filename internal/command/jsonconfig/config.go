...
// provider configurations are the one concept in Ghoten that can span across
// module boundaries.
...
func marshalProviderConfigs(
	...
	for k, pc := range c.Module.ProviderConfigs {
		...
		schema := mapSchema(schemas, func(schemas *ghoten.Schemas) *configschema.Block {
...
func marshalModule(c *configs.Config, schemas *ghoten.Schemas, addr string) (module, error) {
	var module module
	var rs []resource
...
func marshalModuleCalls(c *configs.Config, schemas *ghoten.Schemas) map[string]moduleCall {
...
func marshalModuleCall(c *configs.Config, mc *configs.ModuleCall, schemas *ghoten.Schemas) moduleCall {
...
func marshalResources(resources map[string]*configs.Resource, schemas *ghoten.Schemas, moduleAddr string) ([]resource, error) {
	var rs []resource
	for _, v := range resources {
		...
		if v.Managed != nil && len(v.Managed.Provisioners) > 0 {
			...
			for _, p := range v.Managed.Provisioners {
				schema := mapSchema(schemas, func(schema *ghoten.Schemas) *configschema.Block {
