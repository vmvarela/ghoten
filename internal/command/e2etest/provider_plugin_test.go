...
// TestProviderProtocols verifies that Ghoten can execute provider plugins
...
func TestProviderProtocols(t *testing.T) {
	...
	tf := e2e.NewBinary(t, ghotenBin, "testdata/provider-plugin")
	...
	simple6ProviderExe := e2e.GoBuild("github.com/vmvarela/ghoten/internal/provider-simple-v6/main", simple6Provider)

	simpleProvider := filepath.Join(tf.WorkDir(), "terraform-provider-simple")
	simpleProviderExe := e2e.GoBuild("github.com/vmvarela/ghoten/internal/provider-simple/main", simpleProvider)
...
func TestProviderGlobalCache(t *testing.T) {
	...
	for i := 0; i < 16; i++ {
		...
		go func() {
			defer wg.Done()
			tf := e2e.NewBinary(t, ghotenBin, "testdata/provider-global-cache")
