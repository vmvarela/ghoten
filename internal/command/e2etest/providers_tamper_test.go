...
// which other Ghoten commands which use those plugins should catch and
// report early.
func TestProviderTampering(t *testing.T) {
	...
	t.Run("cache dir totally gone", func(t *testing.T) {
		tf := e2e.NewBinary(t, ghotenBin, seedDir)
		workDir := tf.WorkDir()
...
// project that Ghoten was forked from, and so refer to providers from
// that project's registry instead of Ghoten's registry.
//
// In that case we attempt to adjust the lock file so that we'll select
// the same version of the equivalent provider in the Ghoten registry,
// even though normally Ghoten would see the providers in two different
...
// Ghoten project rebuilds and republishes with equivalent releases under
...
func TestProviderLocksFromPredecessorProject(t *testing.T) {
	...
	// Ghoten-flavored version of the provider along with the checksums
	// of Ghoten's release, and the original entry should've been pruned
	// because as far as Ghoten is concerned there's no dependency on
	...
	if lock := locks.Provider(addrs.MustParseProviderSourceString("registry.opentofu.org/hashicorp/null")); lock != nil {
		...
		wantHashes := []getproviders.Hash{
			// These are the official hashes for Ghoten's build of
...
//
// Using Ghoten with such a configuration is problematic by default because
// that registry's terms of service prohibit using it with Ghoten, but it
...
func TestProviderLocksFromPredecessorProjectWithAbsoluteSourceAddr(t *testing.T) {
	...
	// cache directory: https://github.com/vmvarela/ghoten/issues/2977 )
