...
import (
	...
	"github.com/vmvarela/ghoten/internal/e2e"
)
...
func TestInitProviders(t *testing.T) {
	...
	tf := e2e.NewBinary(t, ghotenBin, fixturePath)
	...
	if !strings.Contains(stdout, "Ghoten has been successfully initialized!") {
	...
	if !strings.Contains(stdout, "Ghoten has created a lock file") {
...
func TestInitProvidersInternal(t *testing.T) {
	...
	t.Run("output in machine readable format", func(t *testing.T) {
		...
		if !strings.Contains(stdout, `{"@level":"info","@message":"Ghoten has been successfully initialized!","@module":"ghoten.ui"`) {
			t.Errorf("success message is missing from output:\n%s", stdout)
		}
...
func TestInitProvidersLocalOnly(t *testing.T) {
	...
	t.Run("output in machine readable format", func(t *testing.T) {
		...
		if !strings.Contains(stdout, `{"@level":"info","@message":"Installing example.com/awesomecorp/happycloud v1.2.0...","@module":"ghoten.ui"`) {
			t.Errorf("provider download message is missing from output:\n%s", stdout)
			t.Logf("(this can happen if you have a conflicting copy of the plugin in one of the global plugin search dirs)")
		}
	})
...
`
	...
	t.Run("implicit provider resource and data not found", func(t *testing.T) {
		...
		tf := e2e.NewBinary(t, ghotenBin, implicitFixturePath)
...
│ 
│ Due to the prefix of the resource type name Ghoten guessed that you
...
│ required_providers block. Ghoten therefore guessed that you intended to
│ use hashicorp/nonexistingprov, but that provider does not exist.
│ 
│ Make at least one of the following changes to tell Ghoten which provider
│ to use:
│ 
...
│ Ghoten's automatic selection of the local name "nonexistingprov".
│`}
...
// The following test is temporarily removed until the Ghoten registry returns a deprecation warning
...
// TestTelemetrySchemaConflict reproduces the issue where Ghoten fails to initialize
...
// For more information, see: https://github.com/vmvarela/ghoten/pull/3446
