...
func TestMocksAndOverrides(t *testing.T) {
	...
	tf := e2e.NewBinary(t, ghotenBin, filepath.Join("testdata", "overrides-in-tests"))
...
// a fix for this issue https://github.com/vmvarela/ghoten/issues/3644
//
// The bug occurs when:
...
func TestMockProviderComputedBlockCleanup(t *testing.T) {
	...
	tf := e2e.NewBinary(t, ghotenBin, filepath.Join("testdata", "mock-computed-block-cleanup"))
	...
	if err != nil {
		if strings.Contains(stdout, "Config value can not be specified for computed field") {
			t.Errorf("Bug reproduced: mock provider fails with computed field error.\n"+
				"This is the bug from https://github.com/vmvarela/ghoten/issues/3644\n"+
				"stdout:\n%s", stdout)
			return
		}
