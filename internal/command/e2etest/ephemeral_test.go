...
func TestEphemeralErrors_variables(t *testing.T) {
	tf := e2e.NewBinary(t, ghotenBin, "testdata/ephemeral-errors/variables")
	...
	{ // run with validation failures
		...
		if !strings.Contains(sanitized, `The error expression used to explain this condition refers to ephemeral values. Ghoten will not display the resulting message.  You can correct this by removing references to ephemeral values or by utilizing the builtin ephemeralasnull() function.`) {
...
func TestEphemeralErrors_outputs(t *testing.T) {
	tf := e2e.NewBinary(t, ghotenBin, "testdata/ephemeral-errors/outputs")
	buildSimpleProvider(t, "6", tf.WorkDir(), "simple")
	with := func(path string, fn func()) {
		src := tf.Path(path + ".disabled")
		dst := tf.Path(path)
	...
	with("ephemeral_output_with_precondition.tf", func() {
		...
		if !strings.Contains(sanitizedSout, `Warning: Error message refers to ephemeral values    on `+filename) ||
			!strings.Contains(sanitizedSout, `The error expression used to explain this condition refers to ephemeral values, so Ghoten will not display the resulting message.  You can correct this by removing references to ephemeral values`) {
