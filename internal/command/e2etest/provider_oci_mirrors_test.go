...
import (
	...
	"github.com/vmvarela/ghoten/internal/command/e2etest/fakeocireg"
...
// but it does at least test the part of the behavior that lives inside Ghoten
...
func TestProviderOCIMirrors(t *testing.T) {
	...
	// to access servers run by the Ghoten project when TF_ACC=1 is set in the
	// environment. However, the Ghoten project does not currently run an
	...
	tf := e2e.NewBinary(t, ghotenBin, "testdata/oci-provider-mirror")
