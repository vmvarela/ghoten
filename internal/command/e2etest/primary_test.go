...
func TestPrimarySeparatePlan(t *testing.T) {
	...
	if !strings.Contains(stdout, "ghoten apply \"tfplan\"") {
...
func TestEphemeralWorkflowAndOutput(t *testing.T) {
	...
	pluginVersionRunner := func(t *testing.T, testdataPath string, providerBuilderFunc func(*testing.T, string)) {
		tf := e2e.NewBinary(t, ghotenBin, testdataPath)
		...
		{ // PLAN
			...
			expectedChangesOutput := `Ghoten used the selected providers to generate the following execution plan.
...
Changes to Outputs:
  + final_output = "just a simple resource to ensure that the second provider it's working fine"`
		...
		{ // APPLY with no ephemeral variable value
			expectedToContain := "╷ Error: No value for required variable    on main.tf line 15:   15: variable \"ephemeral_input\" {  Variable \"ephemeral_input\" is configured as ephemeral. This type of variables need to be given a value during `ghoten plan` and also during `ghoten apply`.╵"
			expectedErr := fmt.Errorf("exit status 1")
...
func buildSimpleProvider(t *testing.T, version string, workdir string, buildOutName string) {
	...
	providerTmpBinPath := e2e.GoBuild(fmt.Sprintf("github.com/vmvarela/ghoten/internal/%s/main", implPkgName), providerBuildOutDir)
