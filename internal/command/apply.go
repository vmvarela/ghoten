...
// ApplyCommand is a Command implementation that applies a Ghoten
...
func (c *ApplyCommand) Run(rawArgs []string) int {
	...
	// rendered already in a remote Ghoten process.
	...
	if lp, ok := planFile.Local(); ok {
		...
		if plan.Backend.Config == nil {
			...
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				"Failed to read plan from plan file",
				"The given plan file does not have a valid backend configuration. This is a bug in the Ghoten command that generated this plan file.",
			))
			return nil, diags
		}
...
Options:
  ...
  -destroy                     Destroy Ghoten-managed infrastructure.
                               The command "ghoten destroy" is a convenience alias
                               for this option.
  ...
  all of the plan-customization options accepted by the ghoten plan command.
...
Options:
  ...
  the ghoten plan command. For more information on those options, run:
      ghoten plan -help
`
	return strings.TrimSpace(helpText)
}
