...
func (c *GraphCommand) Run(args []string) int {
	...
	case "apply":
		...
		// Historically "ghoten graph" would allow the nonsensical request to
	...
	graphStr, err := ghoten.GraphDot(g, &dag.DotOpts{
...
func (c *GraphCommand) Help() string {
	helpText := `
Usage: ghoten [global options] graph [options]
...
Options:
  ...
  -type=plan       Type of graph to output. Can be: plan, plan-refresh-only,
                   plan-destroy, or apply. By default Ghoten chooses
				   "plan", or "apply" if you also set the -plan=... option.

  -module-depth=n  (deprecated) In prior versions of Ghoten, specified the
