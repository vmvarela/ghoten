...
// contents of a Ghoten plan or state file.
...
func (c *ShowCommand) Run(rawArgs []string) int {
	...
	if showDiags.HasErrors() {
		// "ghoten show" intentionally ignores warnings unless there is at
...
Usage: ghoten [global options] show [target-selection-option] [other-options]
  ...
  Reads and outputs a Ghoten state or plan file in a human-readable
...
// "ghoten show" methods that may or may not have a state file to use.
func (c *ShowCommand) maybeGetSchemas(ctx context.Context, stateFile *statefile.File, config *configs.Config) (*ghoten.Schemas, tfdiags.Diagnostics) {
...
func (c *ShowCommand) showConfiguration(ctx context.Context) (showRenderFunc, tfdiags.Diagnostics) {
	...
	if err != nil {
		diags = diags.Append(tfdiags.Sourceless(
			...
			fmt.Sprintf("Ghoten encountered an unexpected error while verifying that the given configuration directory is valid: %s.", err),
		))
		return nil, diags
	}
	if empty {
		diags = diags.Append(tfdiags.Sourceless(
			...
			"This directory contains no Ghoten configuration files.",
		))
		return nil, diags
	}
