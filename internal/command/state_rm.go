...
func (c *StateRmCommand) Run(args []string) int {
	...
	if isCount == 0 {
		diags = diags.Append(tfdiags.Sourceless(
			...
			"No matching objects found. To view the available instances, use \"ghoten state list\". Please modify the address to reference a specific instance.",
		))
		c.showDiagnostics(diags)
		return 1
	}
...
func (c *StateRmCommand) Help() string {
	helpText := `
Usage: ghoten [global options] state (remove|rm) [options] ADDRESS...

  Remove one or more items from the Ghoten state, causing Ghoten to
  "forget" those items without first destroying them in the remote system.

  This command removes one or more resource instances from the Ghoten state
  based on the addresses given. You can view and list the available instances
  with "ghoten state list".
...
Options:
  ...
  -backup=PATH            Path where Ghoten should write the backup
                          state.
  ...
  -ignore-remote-version  Continue even if remote and local Ghoten versions
