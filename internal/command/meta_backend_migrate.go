...
import (
	...
	"github.com/vmvarela/ghoten/internal/backend/remote"
...
func (m *Meta) backendMigrateState(ctx context.Context, opts *backendMigrateOpts) error {
	...
	// Disregard remote Ghoten version for the state source backend. If it's a
	...
	// Disregard remote Ghoten version if instructed to do so via CLI flag.
	if m.ignoreRemoteVersion {
		m.ignoreRemoteVersionConflict(opts.Destination)
	} else {
		// Check the remote Ghoten version for the state destination backend. If
...
func (m *Meta) backendMigrateState_S_S(ctx context.Context, opts *backendMigrateOpts) error {
	...
	if !migrate {
		...
		migrate, err = m.confirm(&ghoten.InputOpts{
...
func (m *Meta) backendMigrateEmptyConfirm(source, destination statemgr.Full, opts *backendMigrateOpts) (bool, error) {
	var inputOpts *ghoten.InputOpts
	if opts.DestinationType == "cloud" {
		inputOpts = &ghoten.InputOpts{
...
func (m *Meta) promptMultiStateMigrationPattern(sourceType string) (string, error) {
	...
	renameWorkspaces, err := m.UIInput().Input(context.Background(), &ghoten.InputOpts{
	...
	pattern, err := m.UIInput().Input(context.Background(), &ghoten.InputOpts{
...
Prior to changing backends, Ghoten inspects the source and destination
states to determine what kind of migration steps need to be taken, if any.
Ghoten failed to load the states. The data in both the source and the
...
Ghoten copies workspaces in alphabetical order. Any workspaces
...
Unlike typical Ghoten workspaces representing an environment associated with a particular
...
As part of migrating to cloud backend, Ghoten can optionally copy your
...
Should Ghoten migrate your existing state?
`

const tfcInputBackendMigrateRemoteMultiToCloud = `
When migrating from the 'remote' backend to Ghoten's native integration
with cloud backend, Ghoten will automatically create or use existing
workspaces based on the previous backend configuration's 'prefix' value.

When the migration is complete, workspace names in Ghoten will match the
...
workspaces. If you continue, Ghoten will copy your current workspace %[3]q
...
up, or cancel altogether, answer "no" and Ghoten will abort.
`

const inputBackendMigrateMultiToMulti = `
Both the existing %[1]q backend and the newly configured %[2]q backend
support workspaces. When migrating between backends, Ghoten will copy
all workspaces (with the same names). THIS WILL OVERWRITE any conflicting
states in the destination.

Ghoten initialization doesn't currently migrate only select workspaces.
If you want to migrate a select number of workspaces, you must manually
pull and push those states.

If you answer "yes", Ghoten will migrate all states. If you answer
"no", Ghoten will abort.
`
