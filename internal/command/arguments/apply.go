...
func ParseApply(args []string) (*Apply, func(), tfdiags.Diagnostics) {
	...
	if apply.ViewOptions.jsonFlag && apply.PlanPath == "" && !apply.AutoApprove {
		diags = diags.Append(tfdiags.Sourceless(
			...
			"Ghoten cannot ask for interactive approval when -json is set. You can either apply a saved plan file, or enable the -auto-approve option.",
		))
	}
...
// "ghoten destroy" command, which is effectively an alias for
// "ghoten apply -destroy".
func ParseApplyDestroy(args []string) (*Apply, func(), tfdiags.Diagnostics) {
	...
	// and -refresh-only to determine the plan mode. For "ghoten destroy"
	...
	case plans.DestroyMode:
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Error,
			"Invalid mode option",
			"The -destroy option is not valid for \"ghoten destroy\", because this command always runs in destroy mode.",
		))
	case plans.RefreshOnlyMode:
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Error,
			"Invalid mode option",
			"The -refresh-only option is not valid for \"ghoten destroy\".",
		))
	default:
		...
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Error,
			"Invalid mode option",
			fmt.Sprintf("The \"ghoten destroy\" command doesn't support %s.", apply.Operation.PlanMode),
		))
	}
