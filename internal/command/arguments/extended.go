...
// DefaultParallelism is the limit Ghoten places on total parallel
// operations as it walks the dependency graph.
const DefaultParallelism = 10

// State describes arguments which are used to define how Ghoten interacts
// with state.
type State struct {
...
// Operation describes arguments which are used to configure how a Ghoten
	...
	case o.refreshOnlyRaw:
		...
		if o.RefreshMode == plans.RefreshNone {
			diags = diags.Append(tfdiags.Sourceless(
				...
				"It doesn't make sense to use -refresh-only at the same time as -refresh=false, because Ghoten would have nothing to do.",
			))
		}
	default:
