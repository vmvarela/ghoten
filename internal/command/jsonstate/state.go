...
import (
	...
	"github.com/vmvarela/ghoten/internal/command/jsonchecks"
...
func MarshalForRenderer(sf *statefile.File, schemas *ghoten.Schemas) (Module, map[string]Output, error) {
...
func MarshalForLog(sf *statefile.File, schemas *ghoten.Schemas) (*State, error) {
	output := newState()
...
func Marshal(sf *statefile.File, schemas *ghoten.Schemas) ([]byte, error) {
...
func (jsonstate *State) marshalStateValues(s *states.State, schemas *ghoten.Schemas) error {
	var sv StateValues
	var err error
...
func marshalRootModule(s *states.State, schemas *ghoten.Schemas) (Module, error) {
	var ret Module
	var err error
...
func marshalResources(resources map[string]*states.Resource, module addrs.ModuleInstance, schemas *ghoten.Schemas) ([]Resource, error) {
	var ret []Resource
	...
	for _, r := range sortedResources {
		...
		for _, k := range sortedKeys {
			...
			case addrs.EphemeralResourceMode:
				return ret, fmt.Errorf("ephemeral resource %q detected in the current state. This is an error in Ghoten", resAddr.String())
			default:
