...
func TestShow_planErrored(t *testing.T) {
	...
	got := output.Stdout()
	want := `Planning failed. Ghoten encountered an error while generating this plan.`
	if !strings.Contains(got, want) {
		t.Fatalf("unexpected output\ngot: %s\nwant: %s", got, want)
	}
}
...
// Ghoten Core.
func showFixtureProvider() *ghoten.MockProvider {
	p := testProvider()
...
// GetSchemaResponse, PlanResourceChangeFn, and ApplyResourceChangeFn populated,
// with the plan/apply steps just passing through the data determined by
// Ghoten Core. It also has a sensitive attribute in the provider schema.
func showFixtureSensitiveProvider() *ghoten.MockProvider {
	p := testProvider()
...
func TestShow_config_noArgs(t *testing.T) {
	...
	got := output.Stderr()
	want := "This directory contains no Ghoten configuration files."
	if !strings.Contains(got, want) {
		t.Errorf("unexpected output\ngot: %s\nwant:\n%s", got, want)
	}
}
...
func TestShow_module(t *testing.T) {
	...
	// be used by the Ghoten module registry indexing process.
