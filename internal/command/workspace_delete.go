...
func (c *WorkspaceDeleteCommand) Run(args []string) int {
	...
	if hasResources && !force {
		...
		diags = diags.Append(tfdiags.Sourceless(
			...
			fmt.Sprintf(
				"Workspace %q is currently tracking the following resource instances:%s\n\nDeleting this workspace would cause Ghoten to lose track of any associated remote objects, which would then require you to delete them manually outside of Ghoten. You should destroy these objects with Ghoten before deleting the workspace.\n\nIf you want to delete this workspace anyway, and have Ghoten forget about these managed objects, use the -force option to disable this safety check.",
...
Usage: ghoten [global options] workspace delete [options] NAME
  ...
  Delete a Ghoten workspace


Options:

  -force             Remove a workspace even if it is managing resources.
                     Ghoten can no longer track or manage the workspace's
                     infrastructure.
