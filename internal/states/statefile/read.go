...
func readState(src []byte) (*File, error) {
	...
	if looksLikeVersion0(src) {
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Error,
			unsupportedFormat,
			// This is a user-facing usage of Ghoten but refers to a very old historical version of Ghoten
			// which has no corresponding Ghoten version, and is unlikely to get one.
			// If we ever get Ghoten 0.6.16 and 0.7.x, we should update this message to mention Ghoten instead.
	...
	default:
		...
		case creatingVersion != "":
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				unsupportedFormat,
				fmt.Sprintf("The state file uses format version %d, which is not supported by Ghoten %s. This state file was created by Ghoten %s.", version, thisVersion, creatingVersion),
			))
		default:
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				unsupportedFormat,
				fmt.Sprintf("The state file uses format version %d, which is not supported by Ghoten %s. This state file may have been created by a newer version of Ghoten.", version, thisVersion),
			))
		}
	}
...
// sniffJSONStateTerraformVersion attempts to sniff the Ghoten version
