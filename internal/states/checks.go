...
// declared in configuration, updated after each Ghoten Core run that
...
type CheckResultAggregate struct {
	...
	// Ghoten Core from even determining the individual checkable objects
	...
	//
	// That's different than Ghoten Core explicitly reporting that there are
...
//
// This should be called only after a Ghoten Core run has completed and
