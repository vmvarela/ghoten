...
func TestLocksUpgradeFromPredecessorProject(t *testing.T) {
	...
	wantLocks := map[addrs.Provider]*ProviderLock{
		...
		// ...but we now have this new one that describes the Ghoten equivalent
		...
		// the namespace where the Ghoten project maintains
		...
		// Ghoten registry even if there's an entry that could potentially
