...
type Locks struct {
	...
	// this particular run due to some feature that forces Ghoten to not
	...
	// part of a lock file and thus not persistent between Ghoten runs.
...
// SetProviderOverridden records that this particular Ghoten process will
...
// like they were probably created by the project that Ghoten forked from,
// and if so adds similar lock entries for same-named providers in the Ghoten
// registry.
//
...
func (l *Locks) UpgradeFromPredecessorProject() map[addrs.Provider]addrs.Provider {
	...
	for oldAddr, oldLock := range l.providers {
		if oldAddr.Hostname != svchost.Hostname("registry.terraform.io") || oldAddr.Namespace != "hashicorp" {
			...
			// in the Ghoten registry that are controlled by the Ghoten
...
// are included only so Ghoten can detect if the constraints in
...
// of Ghoten considers the strongest of the available hashing schemes, one
