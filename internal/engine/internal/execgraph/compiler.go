...
import (
	...
	"github.com/vmvarela/ghoten/internal/lang/grapheval"
	...
	for opIdx, opDesc := range c.sourceGraph.ops {
		...
		default:
			c.diags = c.diags.Append(tfdiags.Sourceless(
				...
				fmt.Sprintf("Execution graph includes %s, but the compiler doesn't know how to handle it. This is a bug in Ghoten.", opDesc.opCode),
			))
			continue
		}
...
func (c *compiler) compileResultRef(ref AnyResultRef) nodeExecuteRaw {
	...
	case anyOperationResultRef:
		...
		return func(ctx context.Context) (any, bool, tfdiags.Diagnostics) {
			...
			if err != nil {
				...
				// during the apply phase is always a bug in Ghoten because
	...
	default:
		c.diags = append(c.diags, tfdiags.Sourceless(
			tfdiags.Error,
			errSummary,
			fmt.Sprintf("The execution graph includes %#v, but the compiler doesn't know how to handle it. This is a bug in Ghoten.", ref),
		))
		return nil
	}
}
