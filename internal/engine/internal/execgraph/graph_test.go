...
import (
	...
	"github.com/vmvarela/ghoten/internal/engine/internal/execgraph/execgraphproto"
)
...
func TestGraphMarshalUnmarshalValid(t *testing.T) {
	...
	// Ghoten with a different version of Ghoten, so it's not justified
	...
	tests := map[string]struct {
		...
		// expect to be parsing graphs produced by Ghoten itself, and so any
		// errors we encounter are either bugs in Ghoten or caused by
		// something outside of Ghoten tampering with the serialized graph.
		...
		// only really needs to support graphs that Ghoten's planning engine
