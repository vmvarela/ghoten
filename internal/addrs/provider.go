...
// from Ghoten's predecessor, we currently exclusively use the "transitional"
// builtin provider host that matches what the predecessor used, thereby
// helping with cross-compatibility. If we introduce any Ghoten-specific
...
// uses a hostname that belongs to the Ghoten project.
...
// which remains only for compatibility with older Ghoten versions.
func ImpliedProviderForUnqualifiedType(typeName string) Provider {
	...
	case "terraform":
		...
		// otherwise older versions of Ghoten could implicitly select
