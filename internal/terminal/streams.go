...
// terminal to meet the assumptions that the rest of Ghoten makes.
//
// Specifically, Ghoten requires a Terminal which supports virtual terminal
...
// to a terminal. For example, this happens if the user runs Ghoten with
...
// Init tries to initialize a terminal, if Ghoten is running in one, and
...
// can't meet Ghoten's assumptions. For example, on Windows Init will return
// an error if Ghoten is running in a Windows Console that refuses to
