...
import (
	...
	"github.com/vmvarela/ghoten/internal/command"
...
const (
	...
	EnvCPUProfile = "GHOTEN_CPU_PROFILE"
)
...
func realMain() int {
	...
	ctx, span := tracing.Tracer().Start(ctx, "ghoten")
	defer span.End()
	...
	if fips140.Enabled() {
		log.Printf("[WARN] Go runtime FIPS 140-3 mode is enabled; Ghoten is not supported in this configuration, which may cause undesirable behavior")
	}
	...
	if len(diags) > 0 {
		...
		if diags.HasErrors() {
			...
			// We continue to run anyway, since Ghoten has reasonable defaults.
		}
	}
	...
	// Ghoten's behalf using this environment variable. This is used
	...
	if cmd := cliRunner.Subcommand(); cmd != "" && !autoComplete {
		...
		// "ghoten state push", cmd would be "state" here and thus would
	...
	// We might generate some additional log lines if Ghoten relied on any
